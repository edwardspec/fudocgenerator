 ...
 * registry.add( item1 ); registry.add( item2 ); registry.add( item3 );
 * // ...
...
class PageNameRegistry {
			...
			if ( lazyDispute ) {
				...
				if ( lazyDispute.counter > 1 ) {
					util.log( '[info] PageNameRegistry: lazy title allocation: ' + result + ': ' +
						( arbitraryObject.itemCode || arbitraryObject.type || arbitraryObject.biomeCode ) );
				}
			}
		}
	...
	ensureEverythingIsLoaded() {
		...
		for ( var dbName of essentialDatabases ) {
			var db = lib[dbName];
			if ( !db.loaded ) {
	...
	/**
	 * Returns override value (if it exists) or fallbackValue if it doesn't.
	 *
	 * @param {string} configName Configuration variable with overrides list, e.g. "overrideMonsterPageTitles".
	 * @param {string} key Key in the overrides list, e.g. item ID, monster type or biome code.
	 * @param {string} fallbackValue What to return if there is no override for this key.
	 * @return {string}
	 */
	getOverride( configName, key, fallbackValue ) {
		if ( !config[configName] ) {
			return fallbackValue;
		}

		var override = config[configName][key];
		if ( override && override === fallbackValue ) {
			util.log( '[warning] PageNameRegistry: unnecessary override: ' + configName + '["' + key + '"]' );
		}
	...
	resolve() {
		...
		for ( var arbitraryObject of this.objects ) {
			...
			if ( className === 'Item' ) {
				...
				info.isItem = true;
				info.wantedTitle = this.getOverride( 'overrideItemPageTitles', item.itemCode, item.displayName );
				...
				info.isMonster = true;
				info.wantedTitle = this.getOverride( 'overrideMonsterPageTitles', monster.type, monster.displayName );
				...
				info.isBiome = true;
				info.wantedTitle = this.getOverride( 'overrideBiomePageTitles', biome.biomeCode, biome.displayName );
			} else {
		...
		var stackWhenResolveWasCalled = new Error().stack.replace( /[^\n]+/, '' );
		this.add = () => {
