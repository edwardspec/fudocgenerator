...
class PageNameRegistry {
	...
	resolve() {
		...
		for ( var [ wantedTitle, candidates ] of titleCandidates ) {
			if ( candidates.length < 2 ) {
				...
				this.objectToTitle.set( candidates[0].object, wantedTitle );
	...
	attemptResolvingPass( titleCandidates ) {
		...
		for ( var [ wantedTitle, candidates ] of titleCandidates ) {
			...
			for ( var info of candidates ) {
				// FIXME: code duplication, there is similar code in resolve().
				var wantedTitle = info.wantedTitle;
				var competitors = newTitleCandidates.get( wantedTitle );
				if ( !competitors ) {
					competitors = [];
					newTitleCandidates.set( wantedTitle, competitors );
				}
				competitors.push( info );
			}
		}
		...
		for ( var [ wantedTitle, candidates ] of newTitleCandidates ) {
			...
			if ( candidates.length < 2 ) {
				...
				// Since this object was successfully named, remove it from titleCandidates.
				// Only the titles with conflicts will remain.
				newTitleCandidates.delete( wantedTitle );
				continue;
			}
		}

		var remainingTitles = [...newTitleCandidates.keys()];
		util.log( '[notice] PageNameRegistry: processed ' + titleCandidates.size + ' collisions. ' +
			'Still remaining collisions (' + remainingTitles.length + '): ' + remainingTitles.sort().join( ', ' ) );

		if ( titleCandidates.size === remainingTitles.length ) {
