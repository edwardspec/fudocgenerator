...
const { config, RemoveBadSymbols, util } = require( '.' );
...
class PageNameRegistry {
	...
	getTitleFor( arbitraryObject, flags = {} ) {
		...
		let result = this.objectToTitle.get( arbitraryObject );
		if ( !result && !flags.noLazyAllocation ) {
			const lazyDispute = this.objectToLazyDispute.get( arbitraryObject );
			if ( lazyDispute ) {
				while ( true ) {
	...
	getObjectByTitle( pageName, requiredType = null ) {
		...
		const foundObject = this.titleToObject.get( pageName );
	...
	/**
	 * Load any databases that are known to call PageNameRegistry.add().
	 * We call this from resolve(), because using add() is no longer allowed after resolve().
	 */
	ensureEverythingIsLoaded() {
		const essentialDatabases = [
			'ItemDatabase',
			'MonsterDatabase',
			'BiomeDatabase',
			'SaplingDatabase',
			'TreasurePoolDatabase'
		];
		const lib = require( '.' );

		for ( const dbName of essentialDatabases ) {
			const db = lib[dbName];
			if ( !db.loaded ) {
				util.log( '[info] PageNameRegistry: ' + dbName + ' wasn\'t loaded before resolve(), loading it now.' );
				db.load();
			}
		}
	}
	 ...
	 */
	resolve() {
		// This is the last moment when calling PageNameRegistry.add() is still allowed,
		// so let's load any databases that are known to call PageNameRegistry.add().
		this.ensureEverythingIsLoaded();
		...
		const allCandidates = [];
		for ( const arbitraryObject of this.objects ) {
			const info = {
				object: arbitraryObject
			};

			const className = arbitraryObject.constructor.name;
			if ( className === 'Item' ) {
				const item = arbitraryObject;

				info.isItem = true;
				info.wantedTitle = config.overrideItemPageTitles[item.itemCode] || item.displayName;
			} else if ( className === 'Monster' ) {
				const monster = arbitraryObject;

				if ( monster.type.startsWith( 'bee_' ) ) {
					// We don't want separate articles about free-flying bee monsters,
					// because the readers will be confused by 3 articles "<type> Queen", "<type> Drone" and "<type> Bee".
					continue;
				}
				...
				const pool = arbitraryObject;

				info.isPool = true;
				info.wantedTitle = 'TreasurePool:' + util.ucfirst( pool.name );
			} else if ( className === 'SaplingPart' ) {
				const saplingPart = arbitraryObject;

				info.isSapling = true;
				info.wantedTitle = 'Template:Modular tree part/' + ( saplingPart.isFoliage ? 'Foliage' : 'Stem' ) +
					'/' + saplingPart.name;
			} else if ( className === 'Biome' ) {
				const biome = arbitraryObject;

				info.isBiome = true;
				info.wantedTitle = config.overrideBiomePageTitles[biome.biomeCode] || biome.displayName;
			} else {
			...
			info.wantedTitle = RemoveBadSymbols.fromPageName( info.wantedTitle );
		...
		const titleCandidates = this.groupCandidatesByTitle( allCandidates );
		...
		for ( const candidates of titleCandidates.values() ) {
			for ( const info of candidates ) {
				if ( info.isItem ) {
					...
					const racePrefixMatch = item.itemCode.match( /(apex|avian|floran|glitch|human|hylotl|novakid|protectorate)/ );
					if ( racePrefixMatch ) {
						info.racePrefix = util.ucfirst( racePrefixMatch[1] );
					}

					if ( item.smashable || item.smashOnBreak ) {
						info.smashable = true;
					}

					if ( item.asset.filename.startsWith( 'objects/unsmashable' ) ) {
						info.unsmashable = true;
					}
					...
					const elementPrefixMatch = monster.type.match( /^(?:fu|)(poison|fire|ice|shadow|electric)/ );
		...
		// Sanity check: after resolve() all titles are chosen and final, so PageNameRegistry.add() won't do anything.
		const stackWhenResolveWasCalled = new Error().stack.replace( /[^\n]+/, '' );
		this.add = () => {
			console.log( 'Error: PageNameRegistry.add() was called after resolve(). Stack trace from when resolve() was called: ' + stackWhenResolveWasCalled );
	...
	groupCandidatesByTitle( candidates, targetMap ) {
		...
		for ( const info of candidates ) {
			const wantedTitle = info.wantedTitle;
			let competitors = targetMap.get( wantedTitle );
			if ( !competitors ) {
				competitors = [];
	...
	allocateUndisputedTitles( titleCandidates ) {
		for ( const [ wantedTitle, candidates ] of titleCandidates ) {
	...
	attemptResolvingPass( titleCandidates ) {
		const newTitleCandidates = new Map();

		// Attempt to resolve the conflicts.
		for ( let candidates of titleCandidates.values() ) {
		...
		if ( titleCandidates.size === newTitleCandidates.size ) {
			...
			for ( const [ wantedTitle, candidates ] of newTitleCandidates ) {
				const lazyDispute = {
	...
	attemptRenameRecursive( candidates ) {
		const wantedTitle = candidates[0].wantedTitle;
		for ( let i = 0; i < candidates.length - 1; i++ ) {
			for ( let j = i + 1; j < candidates.length; j++ ) {
				...
				if ( renamedCandidate ) {
					...
					const candidatesExceptRenamedOne = candidates.filter( ( x ) => x !== renamedCandidate );
	...
	attemptRename( a, b ) {
		...
		// Detect smashable/unsmashable objects.
		if ( b.smashable && !a.unsmashable ) {
			b.wantedTitle = ' (smashable)';
			return b;
		}

		if ( b.unsmashable && !a.unsmashable && !a.smashable ) {
			b.wantedTitle += ' (Prop Pack)';
			return b;
		}
		...
		// Detect biome name collisions.
		if ( b.isBiome ) {
			if ( !a.isBiome ) {
				// Biome/non-biome collision.
				b.wantedTitle += ' (biome)';
				return b;
			}
		}
