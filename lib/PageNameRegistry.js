...
class PageNameRegistry {
		...
		for ( var candidates of titleCandidates.values() ) {
			for ( var info of candidates ) {
				if ( info.isItem ) {
					...
					var biome = info.object;
					if ( biome.biomeCode.includes( 'aether' ) ) {
						info.isAetherBiome = true;
					}

					if ( biome.biomeCode.includes( 'floor' ) ) {
						info.isOceanFloor = true;
					}
	...
	attemptRename( a, b ) {
		...
		if ( b.isBiome ) {
			...
			// Biome/biome name collision.
			if ( b.isAetherBiome && !a.isAetherBiome ) {
				b.wantedTitle += ' (Aether)';
				return b;
			}

			if ( b.isOceanFloor && !a.isOceanFloor ) {
				b.wantedTitle += ' Floor';
				return b;
			}

			// If all else fails, add a biome code (which is unique).
			// It's much more useful than automatic suffix like (2) or (3).
			b.wantedTitle += ' (' + b.object.biomeCode + ')';
			return b;
		}
