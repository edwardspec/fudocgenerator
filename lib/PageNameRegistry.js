...
class PageNameRegistry {
	...
	getTitleFor( arbitraryObject, flags = {} ) {
		...
		if ( typeof ( result ) === 'function' ) {
			// Lazy assignment, see setLazyFallbackTitle() for details.
			result = result( arbitraryObject, flags.noLazyAllocation );
		}
	 ...
	 * installing a "whoever calls getTitleFor() first gets the title" callback instead of fixed title.
	 *
	 * @param {string} disputedTitle
	 * @param {Object[]} competingObjects
	 *
	 */
	setLazyFallbackTitle( disputedTitle, competingObjects ) {
		this.assertTitleNotOccupied( disputedTitle );

		var fallbackFunc = ( arbitraryObject, noAllocate = false ) => {
			// If noAllocate flag is set, then we return the non-unique "disputedTitle". (see ArmorSet class)
			// If it's not set, we add "(2)", "(3)", etc. to the title, incrementing the counter.
			return noAllocate ? disputedTitle : this.lazyFallback( arbitraryObject, disputedTitle );
		};

		for ( var arbitraryObject of competingObjects ) {
			this.objectToTitle.set( arbitraryObject, fallbackFunc );
		}
		...
		this.titleToObject.set( disputedTitle, competingObjects[0] );
	}

	/**
	 * Lazily assign disputedTitle to the object that asked for it first.
	 * If other objects ask for the same title later, they get a suffix like "(2)", "(3)", etc.
	 *
	 * @param {Object} arbitraryObject Object that requested a title via getTitleFor().
	 * @param {string} disputedTitle Title that this object wants.
	 */
	lazyFallback( arbitraryObject, disputedTitle ) {
		var alreadyAllocatedCount = this.lazyTitleAllocationsCounter.get( disputedTitle ) || 0;
		alreadyAllocatedCount ++;
		this.lazyTitleAllocationsCounter.set( disputedTitle, alreadyAllocatedCount );

		var lazilyChosenTitle = disputedTitle;
		if ( alreadyAllocatedCount > 1 ) {
			lazilyChosenTitle += ' (' + alreadyAllocatedCount + ')';
			util.log( '[info] PageNameRegistry: lazy title allocation: ' + lazilyChosenTitle );
		}

		// We just chose the title for this object, so we won't call this function for it again.
		this.objectToTitle.set( arbitraryObject, lazilyChosenTitle );
		return lazilyChosenTitle;
	}
	...
	attemptResolvingPass( titleCandidates ) {
		...
		if ( titleCandidates.size === newTitleCandidates.size ) {
			...
			// Fallback: for all titles that are still disputed,
			// use lazy logic "which object calls getTitleFor() first will get disputed title".
			for ( var [ wantedTitle, candidates ] of newTitleCandidates ) {
				var competingObjects = candidates.map( ( x ) => x.object );
				this.setLazyFallbackTitle( wantedTitle, competingObjects );
			}
			return;
		}
