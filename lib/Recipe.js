...
class Recipe {
	...
	toCargoDatabase() {
		var result = '',
			rowHeader = '{{#cargo_store:_table = recipe|station_id=' + this.station + '|affected_item_id=',
			rowFooter = '|wikitext=' + this.toWikitext().replace( /\|/g, '{{!}}' ) + '}}\n';

		for ( var ItemCode of Object.keys( this.inputs ) ) {
			result += rowHeader + ItemCode + '|role=input' + rowFooter;
		}

		for ( var ItemCode of Object.keys( this.outputs ) ) {
			result += rowHeader + ItemCode + '|role=output' + rowFooter;
		}
