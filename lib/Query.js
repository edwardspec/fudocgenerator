...
class Query {
	...
	/**
	 * Determine by how much will the mech be fueled by 1 of some item. Returns 0 for non-fuels.
	 * @param {string} itemCode
	 * @return {float}
	 */
	getMechFuelValue( itemCode ) {
		if ( !this.mechFuels ) {
			this.mechFuels = AssetDatabase.getData( 'interface/mechfuel/mechfuel.config' ).fuels;
		}

		var info = this.mechFuels[itemCode];
		return info ? info.fuelMultiplier : 0;
	}
