...
class Query {
	...
	getPlanetName( planetCode ) {
		if ( !this.planetTypeNames ) {
			...
			for ( const [ planetCode2, planetName ] of Object.entries( config.overridePlanetNames ) ) {
	...
	getTileEffects( materialName ) {
		if ( !this.tileEffects ) {
			const tileConf = AssetDatabase.getData( 'tileEffects.config' );
	...
	whichAnimalsEat( itemCode ) {
		if ( !this.itemToAnimalDiet ) {
			...
			const farmConf = AssetDatabase.getData( 'scripts/actions/monsters/farmable.config' ).foodlists;
			for ( const [ diet, listOfFoodItems ] of Object.entries( farmConf ) ) {
				for ( const possibleFoodCode of listOfFoodItems ) {
	...
	liquidNamesToItemCodes( liquidNames ) {
		...
		for ( const liquidName of liquidNames ) {
			...
			const droppedItemCode = this.liquidDb.findByName( liquidName ).itemDrop;
			if ( droppedItemCode ) {
