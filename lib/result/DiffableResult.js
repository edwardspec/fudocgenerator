...
/**
 * If value of the database field is a multiline text with more lines than this number,
 * then diff() will only print the lines that are different, not the entire value.
 */
const MAX_LINES_WITHOUT_TRUNCATING_VALUE = 2;
...
class DiffableResult {
	...
	diff( anotherResult ) {
		...
		for ( var key of [...combinedKeys].sort() ) {
			...
			var whatChanged = oldValue + '\t\t' + newValue;
			if ( whatChanged.split( '\n' ).length > MAX_LINES_WITHOUT_TRUNCATING_VALUE ) {
				...
				whatChanged = '\n\t' + nodeDiff.structuredPatch( '', '', oldValue, newValue, '', '' ).hunks.map( ( hunk ) => {
					return hunk.lines.filter( ( line ) => line[0] === '+' || line[0] === '-' );
				} ).flat().join( '\n\t' );
			}

			diffText += operation + '\t' + key + '\t' + whatChanged + '\n';
		}
