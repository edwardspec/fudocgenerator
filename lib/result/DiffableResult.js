...
class DiffableResult {
	...
	diff( anotherResult ) {
		...
		if ( !process.stdout.isTTY ) {
			// Don't add colors if stdout was redirected into the file.
			colors.disable();
		}
		...
		for ( const key of [...combinedKeys].sort() ) {
			...
			let whatChanged = '';
			if ( oldValue ) {
				whatChanged += colors.red( '- ' + colors.bold( key ) + ' = ' + oldValue ) + '\n';
			}
			if ( newValue ) {
				whatChanged += colors.green( '+ ' + colors.bold( key ) + ' = ' + newValue ) + '\n';
			}
			...
			if ( whatChanged.split( '\n' ).length - 1 > MAX_LINES_WITHOUT_TRUNCATING_VALUE ) {
				...
				whatChanged = colors.bold( key ) + ' ~ modified:\n\t' + nodeDiff.structuredPatch(
					'', '', oldValue || '', newValue || '', '', ''
				).hunks.map( ( hunk ) => {
