...
class DiffableResult {
	...
	/**
	 * Add new precreated/overwritten wikipage into this DiffableResult.
	 *
	 * @param {string} wikiPageName Name of the target page.
	 * @param {string} wikitext Contents of the target page.
	 * @param {boolean} overwrite True if existing page will be overwritten, false otherwise.
	 */
	addPage( wikiPageName, wikitext, overwrite ) {
		const pseudoTableName = overwrite ? '_OverwritePage' : '_PrecreatePage';
		var rows = this.tables.get( pseudoTableName );
		if ( !rows ) {
			rows = new Map();
			this.tables.set( pseudoTableName, rows );
		}

		rows.set( wikiPageName, new Map( [ [ 'text', wikitext ] ] ) );
	}
