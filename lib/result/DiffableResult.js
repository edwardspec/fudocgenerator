...
class DiffableResult {
	 ...
	 */
	diff( anotherResult ) {
		let newData = this.toFlatMap(),
			oldData = anotherResult.toFlatMap(),
			diffText = '';

		const combinedKeys = new Set( [...newData.keys()].concat( [...oldData.keys()] ) );
	...
	toFlatMap() {
		...
		for ( const [ tableName, rows ] of this.tables ) {
			for ( const [ rowIdentifier, row ] of rows ) {
				for ( const [ fieldName, value ] of row ) {
					const key = tableName + '::' + rowIdentifier + '::' + fieldName;
