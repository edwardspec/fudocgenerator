...
class ResultsWriter {
	...
	finalize() {
		...
		if ( argv.diff ) {
			var oldDiffableResult = new DiffableResult();
	...
	chunkOutputCallback( chunkName, contents ) {
		...
		this.overwritePage( pageName, contents, {
			// DiffableResult already shows changes in Cargo rows,
			// so showing a diff of Chunk pages (same information in wikitext format) would be redundant.
	...
	writeToCargo( entity ) {
		...
		for ( var i = 0; i < rows.length; i++ ) {
			...
			if ( argv.diff || argv.save ) {
				this.diffableResult.addRow( cargoRow, partitionKey, entity );
			}
		}
	}
	...
	precreatePage( wikiPageName, wikitext ) {
		...
		this.diffableResult.addPage( wikiPageName, wikitext, false );
	}
	 ...
	 */
	overwritePage( wikiPageName, wikitext, params = {} ) {
		...
		if ( !params.excludeFromDiff ) {
			this.diffableResult.addPage( wikiPageName, wikitext, true );
		}
	}
