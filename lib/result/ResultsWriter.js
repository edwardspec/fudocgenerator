...
const { argv, config, ChunkWriter, DiffableResult, PageNameRegistry, WikiStatusCache } = require( '..' ),
...
class ResultsWriter {
	constructor() {
		this.diffableResultPath = config.outputDir + '/previousResult.v8.serialized';
		...
		this.cargoChunkWriter = new ChunkWriter( {
			groups: {
				...
				itemMetadata: {
					idxPattern: 'itemMetadata/$1',
					chunksCount: config.cargoChunksCount.itemMetadata
				},
				recipe: {
					idxPattern: 'recipe/$1',
				...
				region: {
					idxPattern: 'region/$1',
					chunksCount: config.cargoChunksCount.region
				},
				biome: {
					idxPattern: 'biome/$1',
					chunksCount: config.cargoChunksCount.biome
				},
				weather: {
				...
				function: {
					idxPattern: 'function/$1',
					chunksCount: config.cargoChunksCount.function
				}
			}
		} );
		...
		this.diffableResult = new DiffableResult();
	}
	...
	writeItem( item ) {
		...
		( item.metadata.get( 'tileEffects' ) || [] ).forEach( ( effectCode ) => {
			this.addPrecreatedCategory( 'TileEffect:' + effectCode,
				'[[Category:Blocks by tile effect|' + effectCode + ']]' );
		} );
	}
	...
	/**
	 * Write everything about leveling function.
	 *
	 * @param {LinearClampFunction} func One function from the FunctionDatabase.
	 */
	writeFunction( func ) {
		this.writeToCargo( func );
	}
	...
	getSchemaVersion() {
		if ( !this.schemaVersion ) {
			...
			const versionPath = __dirname + '/../schemaversion.txt';
	...
	finalize() {
		...
		if ( argv.diff ) {
			const oldDiffableResult = new DiffableResult();
			oldDiffableResult.loadFromFile( this.diffableResultPath );

			console.log( this.diffableResult.diff( oldDiffableResult ) );
		}

		if ( argv.save ) {
			this.diffableResult.saveToFile( this.diffableResultPath );
		}
	}
	...
	chunkOutputCallback( chunkName, contents ) {
		const pageName = 'Template:Cargo/' + chunkName;
	 ...
	 */
	writeToCargo( entity ) {
		const partitionKey = entity.getPartitionKey();
		let rows = entity.toCargoDatabase();
		...
		for ( let i = 0; i < rows.length; i++ ) {
			...
			if ( argv.diff || argv.save ) {
				this.diffableResult.addRow( cargoRow, partitionKey );
			}
		}
	}
	...
	writeToArticle( entity ) {
		const wikiPageName = PageNameRegistry.getTitleFor( entity );
		if ( !wikiPageName ) {
			// This object didn't get a PageName.
			return;
		}

		const wikitext = entity.toArticleText( wikiPageName );
	...
	precreatePage( wikiPageName, wikitext ) {
		const wrappedContents = this.wrapPage( wikiPageName, wikitext );
	...
	overwritePage( wikiPageName, wikitext ) {
		const checksum = crypto.createHash( 'sha1' ).update( wikitext.trimEnd(), 'utf8' ).digest( 'hex' );
		const wrappedContents = this.wrapPage( wikiPageName, wikitext );
