...
class ChunkWriter {
	...
	finalizeGroup( groupName, outputCallback ) {
		...
		if ( fs.existsSync( cacheFilename ) ) {
			...
			const cachedChunksCount = cache.get( 'NUMBER_OF_CHUNKS' );
		...
		group.entries.forEach( ( entry, idx ) => {
			const partitionKey = entry.partitionKey;

			// Check the cache first.
			let chunkIndex = cache.get( partitionKey );
			if ( chunkIndex === undefined ) {
				const hash = new Uint8Array( crypto.createHash( 'md5' ).update( partitionKey ).digest() );
		...
		let currentChunkContents = '';

		const writeHeader = ( suffix ) => {
		...
		const loggedSizes = Object.values( bytesWritten ).map( ( a ) => Math.round( ( a || 0 ) / 1024 ) )
			.sort( ( a, b ) => a - b )
			.join( ', ' );
		const loggedRowCounts = Object.values( rowsWritten ).sort( ( a, b ) => a - b ).join( ', ' );
