...
class ResultsWriter {
	...
	writeItem( item ) {
		...
		this.writeIntoCargoDatabase( 'item', 'item-' + itemCode, item.toCargoDatabase() );
	...
	writeRecipe( Recipe ) {
		...
		this.writeIntoCargoDatabase( 'recipe', chunkPartitionKey, Recipe.toCargoDatabase() );
	}
	...
	writeResearchNode( node ) {
		this.writeIntoCargoDatabase( 'node', 'node-' + node.id.split( ':' ).pop(), node.toCargoDatabase() );
	}
	...
	writeArmorSet( armorSet ) {
		this.writeIntoCargoDatabase( 'armorset', 'set-' + armorSet.id, armorSet.toCargoDatabase() );
	}
	 ...
	 * @param {string} Which group of chunks to use (e.g. "node" or "recipe").
	 * @param {string} partitionKey Arbitrary string that determines which Chunk page will be used.
	 * @param {string} wikitext
	 */
	writeIntoCargoDatabase( group, partitionKey, wikitext ) {
		this.cargoChunkWriter.write( group, partitionKey, wikitext );
	}
