...
class ResultsWriter {
	...
	writeIntoCargoDatabase( Recipe ) {
		if ( !this.cargoFd ) {
			this.cargoFd = fs.openSync( config.outputDir + '/cargo.txt', 'w' );
		}

		fs.writeSync( this.cargoFd, Recipe.toCargoDatabase() );
	}
