...
class ResultsWriter {
	...
	/**
	 * Add image to "upload these images" list.
	 * @param {string} ItemCode
	 * @param {string} path
	 */
	 writeToUploadThisList( ItemCode, path ) {
		if ( !this.uploadListFd ) {
			this.uploadListFd = fs.openSync( config.outputDir + '/pywikibot/uploadInventoryIcons.csv', 'w' );
			fs.writeSync( this.uploadListFd, 'description.en,source,author,license,set,name,url\n' );
		}

		var pathWithoutTopDir = path.replace( config.pathToMod, '' ).replace( config.pathToVanilla, '' );

		// Format of this list is CSV file, as expected by "data_ingestion.py" script from pywikibot.
		var csvEntry = [
			// Text description to be written to "File:<something>" page.
			'{{Inventory icon description|' + ItemCode + '}}',
			// Game assets: Source (arbitrary string, so it's ok to use path)
			pathWithoutTopDir,
			// Author, License, Set - pywikibot requires these fields in CSV file, but doesn't use them.
			'', '', '',
			// Filename
			'Item_icon_' + ItemCode + '.png',
			// Source URL. Can be a path on the local filesystem, which is what we have.
			'file://' + path
		];

		fs.writeSync( this.uploadListFd, csvEntry.join( ',' ) + '\n' );
	}
