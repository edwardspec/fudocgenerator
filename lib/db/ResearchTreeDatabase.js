...
class ResearchTreeDatabase {
	...
	load() {
		...
		AssetDatabase.forEach( 'config', ( filename, asset ) => {
			...
			const strings = asset.data.strings;
			...
			for ( var [ treeId, nodes ] of Object.entries( asset.data.researchTree ) ) {
				...
				let treeName = config.overrideResearchTreeName[treeId] || strings.trees[treeId];
				if ( !treeName ) {
				...
				for ( const [ nodeIdWithoutTree, rawNodeData ] of Object.entries( nodes ) ) {
					const [ nodeName, description ] = strings.research[nodeIdWithoutTree];
					if ( !nodeName ) {
					...
					const node = new ResearchNode( rawNodeData, nodeId, treeName, nodeName, description );

					// Remember this node as parent of all node.children.
					node.children = node.children.map( ( childNodeIdWithoutTree ) => {
						const childNodeId = treeId + ':' + childNodeIdWithoutTree;
			...
			for ( const [ childNodeId, parentNodes ] of Object.entries( parents ) ) {
				const childNode = this.knownNodes.get( childNodeId );
	...
	loadDefaultUnlocks() {
		...
		const supportedSpecies = AssetDatabase.getData( '/interface/windowconfig/charcreation.config' ).speciesOrdering;
		supportedSpecies.forEach( ( id ) => {
			const species = AssetDatabase.getData( '/species/' + id + '.species' );
			this.addPseudoNodeWithBlueprints(
				'notree:default-' + id,
	...
	addPseudoNodeWithBlueprints( nodeId, nodeName, defaultBlueprints ) {
		const treeName = 'No', // <-- This becomes "No tree", because "tree" suffix is appended to all trees.
			unlocks = [];

		for ( const blueprints of Object.values( defaultBlueprints ) ) {
			blueprints.forEach( ( element ) => unlocks.push( element.item ) );
		}

		const defaultNode = new ResearchNode(
	...
	nodeListToWikitext( treeName, nodeIdsArray ) {
		return nodeIdsArray.map( ( nodeId ) => {
			const node = this.knownNodesByTree[treeName][nodeId];
			if ( !node ) {
