...
class WeatherPoolDatabase {
	 ...
	 */
	load() {
		const weatherConf = AssetDatabase.getData( '/weather.config' );

		for ( const [ poolName, poolInfo ] of Object.entries( weatherConf ) ) {
	...
	forEach( callback ) {
		...
		for ( const weatherPool of this.knownPools.values() ) {
