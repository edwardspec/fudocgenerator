...
class SaplingDatabase {
	...
	load() {
		...
		const saplingConf = AssetDatabase.getData( 'items/buildscripts/buildsaplingfu.config' );
		...
		AssetDatabase.forEach( 'stem', ( filename, asset ) => {
			const part = new SaplingPart( asset.data, false, this.stemNames[asset.data.name] );
			this.knownStems.set( part.name, part );

			PageNameRegistry.add( part );
		} );
		AssetDatabase.forEach( 'foliage', ( filename, asset ) => {
			const part = new SaplingPart( asset.data, true, this.foliageNames[asset.data.name] );
	...
	getSaplingName( foliageCode, stemCode ) {
		...
		let saplingName = this.stemNames[stemCode] || 'Unknown';
		const foliageName = this.foliageNames[foliageCode];
		if ( foliageName ) {
