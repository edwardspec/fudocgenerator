...
class FunctionDatabase {
	...
	load() {
		AssetDatabase.forEach( 'functions', ( filename, asset ) => {
			for ( const [ functionName, formula ] of Object.entries( asset.data ) ) {
				const [ algorithm, funcType, ...values ] = formula;
				if ( algorithm === 'linear' && funcType === 'clamp' ) {
					const func = new LinearClampFunction( functionName, values );
	...
	calculate( functionName, param ) {
		...
		const func = this.knownFunctions.get( functionName );
		if ( !func ) {
	...
	forEach( callback ) {
		...
		for ( const func of this.knownFunctions.values() ) {
			callback( func );
		}
	}
}
