...
const { ItemDatabase, AssetDatabase, Monster, PageNameRegistry, config, RemoveBadSymbols, util } = require( '..' );
...
class MonsterDatabase {
	 ...
	 */
	load() {
		const ignoredMonstersList = new Set(
			config.ignoredMonsters.concat( Object.keys( config.identicalMonsters ) )
		);

		AssetDatabase.forEach( 'monster', ( filename, asset ) => {
			if ( filename.startsWith( 'monsters/bees/' ) && !filename.match( /cockroach|beetle/ ) ) {
			...
			const monsterCode = monster.type;
		...
		for ( const [ monsterCode, monster ] of this.knownMonsters ) {
			let displayName = monster.shortdescription;
			if ( !displayName ) {
				// The monster may have a capturable variation.
				const capturedMonsterCode = monster.baseParameters.capturedMonsterType;
			...
			if ( !displayName && monster.baseParameters.harvestPool ) {
				...
				for ( const eggItemCode of possibleEggCodes ) {
					const eggName = ItemDatabase.getDisplayName( eggItemCode );
					if ( eggName ) {
			...
			if ( displayName === 'Burrower' ) {
				...
				const match = monsterCode.match( /^(.*)_burrower$/ );
				if ( match ) {
			...
			if ( displayName ) {
				// Successfully named.
				monster.displayName = util.ucfirst( RemoveBadSymbols.fromName( displayName.replace( /_/g, ' ' ) ) );
				...
				if ( monster.isCritter() ) {
					...
					monster.displayName = 'Nameless monster: ' + RemoveBadSymbols.fromName( monsterCode );
		...
		AssetDatabase.forEach( 'monsterpart', ( filename, asset ) => {
			...
			const imageFilename = data.frames.body;
			if ( !imageFilename ) {
	...
	forEach( callback ) {
		...
		for ( const monster of this.knownMonsters.values() ) {
			callback( monster );
		}
	}
