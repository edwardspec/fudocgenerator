...
class StarDatabase {
	 ...
	 */
	load() {
		const celestialConf = AssetDatabase.getData( 'celestial.config' );

		// Map: star codes => human-readable star names,
		// e.g. { "whitestar": "Gentle Star" }
		const starNames = AssetDatabase.getData( 'interface/cockpit/cockpit.config' ).starTypeNames;

		// Map: name of known planetary body type => array of planet IDs,
		// e.g. { "Tier2": [ "forest", "desert" ], ... }
		const knownBodyTypes = new Map();
		for ( const [ bodyType, bodyConf ] of Object.entries( celestialConf.planetaryTypes ) ) {
			knownBodyTypes.set( bodyType, bodyConf.baseParameters.terrestrialType || [] );
		}

		// Same as knownBodyTypes, but for satellites.
		const knownMoonTypes = new Map();
		for ( const [ moonType, moonConf ] of Object.entries( celestialConf.satelliteTypes ) ) {
		...
		for ( const system of Object.values( celestialConf.systemTypes ) ) {
			const starCode = system.baseParameters.typeName;
			const possiblePlanets = new Set();

			for ( const orbit of system.orbitRegions ) {
				for ( const bodyOption of orbit.planetaryTypes ) {
					for ( const planetCode of knownBodyTypes.get( bodyOption.item ) ) {
						possiblePlanets.add( planetCode );
					}
				}

				for ( const moonOption of orbit.satelliteTypes ) {
			...
			for ( const planetCode of possiblePlanets ) {
				let starsWithThisPlanet = this.planetToStars.get( planetCode );
