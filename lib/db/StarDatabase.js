...
class StarDatabase {
	...
	/**
	 * Scan celestial.config and find all stars and their planets.
	 */
	load() {
		...
		for ( var system of Object.values( celestialConf.systemTypes ) ) {
			...
			for ( var orbit of system.orbitRegions ) {
				for ( var bodyOption of orbit.planetaryTypes ) {
					for ( let planetCode of knownBodyTypes.get( bodyOption.item ) ) {
						possiblePlanets.add( planetCode );
					}
				}

				for ( var moonOption of orbit.satelliteTypes ) {
					for ( let planetCode of knownMoonTypes.get( moonOption.item ) ) {
						possiblePlanets.add( planetCode );
					}
				}
			}

			for ( let planetCode of possiblePlanets ) {
		...
		for ( let [ planetCode, nonUniqueStarNames ] of this.planetToStars ) {
