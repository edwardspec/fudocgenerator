...
class ItemDatabase {
	 ...
	 */
	load() {
		const ignoredItemsList = new Set( config.ignoredItems );
		const ignoredUpgradesList = new Set( config.ignoredUpgrades );
		...
		AssetDatabase.forEach( 'item', ( filename, asset ) => {
			...
			for ( const subitem of item.upgradedItems.concat( item.upgradedStations ) ) {
				if ( ignoredUpgradesList.has( subitem.itemCode ) ) {
					util.log( '[info] ItemDatabase: Ignoring ' + subitem.itemCode + ' (in ignoredUpgrades list of config.json).' );
					continue;
				}
	...
	forEach( callback ) {
		...
		for ( const [ itemCode, loadedData ] of this.knownItems ) {
	...
	findCodeByPageName( wikiPageName ) {
		const item = PageNameRegistry.getObjectByTitle( wikiPageName, 'Item' );
