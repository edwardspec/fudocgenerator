...
class ItemDatabase {
	...
	load() {
		...
		var ignoredUpgradesList = new Set( config.ignoredUpgrades );
		...
		AssetDatabase.forEach( 'item', ( filename, asset ) => {
			...
			for ( var subitem of item.upgradedItems.concat( item.upgradedStations ) ) {
		...
		util.log( '[info] ItemDatabase: found ' + this.knownItems.size + ' items in ' +
