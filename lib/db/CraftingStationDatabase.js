...
const { config, ItemDatabase, util } = require( '..' );
...
class CraftingStationDatabase {
	constructor() {
		...
		// Stations that don't require blueprints, e.g. [ "Plating Table", "Prop Pack Truck" ].
		this.stationsWithoutBlueprints = new Set();
	}
	...
	load() {
		...
		ItemDatabase.forEach( ( itemCode, data ) => {
			...
			if ( data.recipeGroup && data.shortdescription ) {
				...
				this.add( data.displayName, [ data.recipeGroup ], true );
			}
			...
			if ( data.interactData && data.interactData.filter ) {
				...
				this.add( data.displayName, data.interactData.filter,
					( data.interactData.requiresBlueprint === false ) );
			}
		} );
	 ...
	 * @param {string[]} groups
	 * @param {boolean} withoutBlueprint True if resulting item doesn't need to be unlocked.
	 */
	add( itemName, groups, withoutBlueprint = false ) {
		...
		if ( withoutBlueprint ) {
			this.stationsWithoutBlueprints.add( itemName );
		}
	}

	/**
	 * Returns true if this crafting station requires resulting item to be unlocked, false otherwise.
	 *
	 * @param {string} stationName
	 * @return {boolean}
	 */
	requiresBlueprint( stationName ) {
		return !this.stationsWithoutBlueprints.has( stationName );
	}
	...
	findByGroups( groups ) {
		...
		for ( var station of config.craftingStationPriorities ) {
			if ( candidates.includes( station ) ) {
				return station;
			}
		}
