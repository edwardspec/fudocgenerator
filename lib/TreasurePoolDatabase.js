...
class TreasurePoolDatabase {
	...
	getPossibleOutputs( poolName, excludeItemCode ) {
		...
		for ( var [ itemCode, averageCount ] of Object.entries( this.knownPools[poolName] || {} ) ) {
			if ( itemCode !== excludeItemCode ) {
				outputs.addItem( itemCode, { averageCount: averageCount } );
			}
		}
	...
	/**
	 * Returns true if item "itemCode" is a part of the pool "poolName", false otherwise.
	 * @param {string} poolName
	 * @param {string} itemCode
	 */
	hasItem( poolName, itemCode ) {
		if ( !this.loaded ) {
			this.load();
		}

		return Object.keys( this.knownPools[poolName] || {} ).includes( itemCode );
	}
}
