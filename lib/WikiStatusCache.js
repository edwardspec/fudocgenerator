...
class WikiStatusCache {
	constructor() {
		...
		if ( fs.existsSync( this.cacheFilename ) ) {
			this._pageExists = new Set( JSON.parse( fs.readFileSync( this.cacheFilename ).toString() ) );
		}
	}
	...
	pageExists( title ) {
		...
		return this._pageExists.has( title );
	}
	...
	async updateCache() {
		...
		console.log( 'updateCache(): found ' + this._pageExists.size + ' existing pages.' );
