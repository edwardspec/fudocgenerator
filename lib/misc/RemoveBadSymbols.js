...
class RemoveBadSymbols {
	...
	replaceColorsWithHtml( origString ) {
		...
		let result = origString.replace( /\^([^;^]+);/g, function ( fullMatch, color ) {
	 ...
	 */
	fromName( origString ) {
		const withoutNewLinesOrLists = origString.replace( //g, '' )
			.replace( /[\r\n]+/g, ' ' )
			.replace( /\s+/g, ' ' );
	...
	fromPageName( origString ) {
		const cleanPageName = this.fromName( origString ).replace( /#/g, 'N' )
			.replace( /\[/g, '(' )
			.replace( /\]/g, ')' );
