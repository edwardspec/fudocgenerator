	...
	loadBiomeMonsters() {
		BiomeDatabase.forEach( ( biome ) => {
			...
			for ( var spawnGroup of spawnProfile.groups ) {
				...
				if ( biome.biomeCode != 'garden' || pool[0][1] != 'bee_honey' ) {
					continue;
				}
				...
				for ( var [ individualChance, spawnTypeName ] of util.normalizeWeights( pool ) ) {
					...
					for ( var roll = 0; roll < selectedCount; roll ++ ) {
						...
						var remainingOtherMonsters = optionsCount - roll - 1;
						...
						console.log( "otherMonstersChance: " + otherMonstersChance );
					}
					...
					console.log( spawnType.monsterType, chance );
