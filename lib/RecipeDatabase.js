...
class RecipeDatabase {
	...
	loadPixelShops() {
		...
		for ( var [ shopName, data ] of Object.entries( shops ) ) {
			data.interactData.items.forEach( ( shopSlot ) => {
				...
				this.add( shopName,
					RecipeSide.newEmpty().addItem( 'money', { count: buyPrice * soldItemCount } ),
					RecipeSide.newEmpty().addItem( soldItemCode, { count: soldItemCount } )
				);
			} );
		}
	}
	...
	/**
	 * Add "fuel item -> produced power" recipes for Quantum Reactor.
	 */
	loadQuantumFuels() {
		var quantumConf = AssetDatabase.getData( 'objects/power/fu_quantumgenerator/fu_quantumgenerator.object' );
		...
		for ( var [ itemCode, fuelPower ] of Object.entries( quantumConf.acceptablefuel ) ) {
			...
			this.add( 'Quantum Reactor', inputs, outputs );
		}
	}

	/**
	 * Add "fuel item -> produced power" recipes for Combustion Generator and Alternator Generator.
	...
	/**
	 * Add "fuel item -> produced power" recipes for Small Fusion Reactor.
	 */
	loadSmallFusionFuels() {
		var smallFusionConf = AssetDatabase.getData( 'objects/power/makeshiftreactor/makeshiftreactor.object' );

		for ( var [ itemCode, fuelOptions ] of Object.entries( smallFusionConf.fuels ) ) {
			var inputs = new RecipeSide();
			inputs.addItem( itemCode, { count: 1 } );
			...
			this.add( 'Small Fusion Reactor', inputs, outputs );
		}
	}
