...
class RecipeDatabase {
	 ...
	 */
	loadStationShops() {
		var stationConf = AssetDatabase.getData( 'interface/scripted/spaceStation/spaceStationData.config' ),
			shopConf = stationConf.shop,
			priceComment = "''(sold for pixels at " + shopConf.initBuyMult + 'x-' +
				shopConf.minBuyMult + "x of the standard price of item)''";
		...
		for ( var [ stationType, itemCodes ] of Object.entries( shopConf.potentialStock ) ) {
			...
			var chanceOfEachItem = 100 * Math.min( 1, avgItems / itemCodes.length );

			var stationName = 'Racial Space Station Terminal (' + ( stationType == 'generic' ? 'any' : stationType ) + ')';
			var inputs = new RecipeSide().addComment( priceComment );

			var outputs = new RecipeSide();
			itemCodes.forEach( ( itemCode ) => outputs.addItem( itemCode, { chance: chanceOfEachItem } ) );
	...
	loadBiomeFish() {
		...
		var rarityChanceMultipliers = fishingConf.rarities.map( function ( rarityInfo ) {
			...
			var chanceMultiplier = Math.min( rarityLevel, 1 ) - previousRarityLevel;
		...
		for ( var [ biomeCode, pools ] of Object.entries( fishingConf.pools ) ) {
			...
			for ( var [ chanceMultiplier, rarity ] of rarityChanceMultipliers ) {
							...
							for ( let [ chance, itemCode ] of util.normalizeWeights( objectSet.pool ) ) {
								...
								if ( isSmashable ) {
									...
									if ( smashPool && !ignoredSmashPools.has( smashPool ) ) {
										...
										smashOutputs.getItemCodes().forEach( ( itemCode ) => {
											itemsDroppedFromBreakableObjects.add( itemCode );
										} );
									}
								}
