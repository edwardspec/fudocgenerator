...
class RecipeDatabase {
	...
	loadExtractionLab() {
		AssetDatabase.getData( 'objects/generic/extractionlab_recipes.config' ).forEach( ( extractorRecipe ) => {
			config.extractorStageBuildings.forEach( ( buildingName, extractorStage ) => {
				const inputs = RecipeSide.newFromExtraction( extractorRecipe.inputs, extractorStage ),
	...
	loadErchiusConverter() {
		for ( const converterItemCode of [ 'precursorconverter', 'precursorconverter2' ] ) {
			var converter = ItemDatabase.find( converterItemCode );
			converter.recipeTable.forEach( ( converterRecipe ) => {
				const outputs = RecipeSide.newFromExtraction( converterRecipe.outputs );
	...
	loadCentrifuges() {
		...
		for ( const [ recipeGroup, buildingName ] of Object.entries( config.centrifugeRecipeGroups ) ) {
			for ( const [ inputItem, outputToRarityMap ] of Object.entries( centrifugeConf[recipeGroup] || {} ) ) {
				...
				for ( const [ outputItem, rarityInfo ] of Object.entries( outputToRarityMap ) ) {
					outputs.addItem( outputItem, { rarity: rarityInfo } );
				}

				const inputs = RecipeSide.newEmpty().addItem( inputItem );
	 ...
	 */
	loadSmelters() {
		for ( const smelterItemCode of [ 'electricfurnace', 'fu_blastfurnace', 'isn_arcsmelter' ] ) {
			const smelter = ItemDatabase.find( smelterItemCode );
			for ( const [ inputItem, outputItem ] of Object.entries( smelter.inputsToOutputs ) ) {
				const bonusOutputs = smelter.bonusOutputs[inputItem] || [];

				const inputs = new RecipeSide();
				inputs.addItem( inputItem, { count: 2 } ); // Base output for smelters is 2 Ore -> 1 Bar.

				const outputs = new RecipeSide();
				outputs.addItem( outputItem, { count: 1 } );

				for ( const [ bonusOutputItem, percent ] of Object.entries( bonusOutputs ) ) {
	...
	loadAtmosphericCondenser() {
		const condenserConf = AssetDatabase.getData( 'objects/power/isn_atmoscondenser/isn_atmoscondenser.object' ),
		...
		const sameOutputBiomes = {};

		for ( const [ biomeCode, pool ] of Object.entries( condenserConf.outputs ) ) {
		...
		for ( const [ biomeCode, outputs ] of Object.entries( outputsPerBiome ) ) {
			...
			const allBiomeCodes = [ biomeCode ].concat( sameOutputBiomes[biomeCode] || [] );
			...
			for ( const biomeCode2 of allBiomeCodes ) {
	...
	loadResourceGenerators() {
		[
			...
			const generatorConf = AssetDatabase.getData( pathToAsset ),
	 ...
	 */
	loadLiquidCollector() {
		const liquidCollectorConf = AssetDatabase.getData( 'objects/power/fu_liquidcondenser/fu_liquidcondenser.object' );
		for ( const [ biomeCode, output ] of Object.entries( liquidCollectorConf.liquids ) ) {
			const liquid = LiquidDatabase.find( output.liquid );
			if ( !liquid ) {
				util.log( '[error] Unknown liquid in outputs of Liquid Collector: ' + output.liquid );
				continue;
			}

			const item = ItemDatabase.find( liquid.itemDrop );
			if ( !item ) {
	...
	loadPixelShops() {
		...
		const lostAndFoundPool = AssetDatabase.getData( 'npcs/merchantpools.config' ).fulostandfound[0][1];
		const lostAndFoundNpc = AssetDatabase.getData( 'npcs/scienceoutpost/lostandfoundnpc.npctype' );
		...
		for ( const [ shopName, data ] of Object.entries( shops ) ) {
			data.interactData.items.forEach( ( shopSlot ) => {
				let soldItemCode = shopSlot.item,
					soldItemCount = 1;
				...
				const soldItem = ItemDatabase.find( soldItemCode );
				if ( !soldItem ) {
				...
				const buyPrice = Math.ceil( ( shopSlot.price || soldItem.price ) * data.interactData.buyFactor );
				...
				const outputs = RecipeSide.newEmpty().addItem( soldItemCode, { count: soldItemCount } );
	 ...
	 */
	loadStationShops() {
		const stationConf = AssetDatabase.getData( 'interface/scripted/spaceStation/spaceStationData.config' ),
			shopConf = stationConf.shop;

		const avgUniqueItems = ( shopConf.minUniqueItems + shopConf.maxUniqueItems ) / 2,
			avgGenericItems = ( shopConf.minGenericItems + shopConf.maxGenericItems ) / 2;

		for ( const [ stationType, itemCodes ] of Object.entries( shopConf.potentialStock ) ) {
			...
			itemCodes.forEach( ( itemCode ) => {
				const soldItem = ItemDatabase.find( itemCode );
				if ( !soldItem ) {
	 ...
	 */
	loadStationGoods() {
		let stationConf = AssetDatabase.getData( 'interface/scripted/spaceStation/spaceStationData.config' ),
		...
		const normalMultiplier = 0.5 * ( goodsNormalRange[0] + goodsNormalRange[1] ),
			abundanceMultiplier = 0.5 * ( goodsAbundanceRange[0] + goodsAbundanceRange[1] ),
			lackMultiplier = 0.5 * ( goodsLackRange[0] + goodsLackRange[1] );

		for ( const goodsOption of stationConf.goods ) {
			const { baseAmount } = goodsOption;
	...
	loadHarvestableItems() {
		ItemDatabase.forEach( ( itemCode, data ) => {
			...
			let station, inputs = new RecipeSide();
			...
			const stageMinDurations = [],
				stageMaxDurations = [];

			for ( const stage of data.stages ) {
				const poolName = stage.harvestPool;
				if ( !poolName ) {
					const duration = stage.duration || [ 0, 0 ];
					stageMinDurations.push( duration[0] );
					stageMaxDurations.push( duration[1] );
				} else {
					let outputs = TreasurePoolDatabase.getPossibleOutputs( poolName );
					let yieldsNoSeeds = false;
					...
					const isPerennial = ( stage.resetToStage !== undefined ),
						nextStage = stage.resetToStage || 0;

					let firstHarvestComment = '';
					if ( nextStage > 0 ) {
	...
	loadMonsterDrops() {
		...
		MonsterDatabase.forEach( ( monster ) => {
			...
			if ( monster.dropPools.length > 0 ) {
				...
				const poolOptions = monster.dropPools[0];
			...
			for ( const dropInfo of pools ) {
				const poolName = dropInfo.poolName;
				if ( !poolName || poolName === 'empty' ) {
					continue;
				}

				const station = dropInfo.station;
				let inputs = poolToInputs[station][poolName];
				if ( inputs ) {
				...
				const outputs = TreasurePoolDatabase.getPossibleOutputs( poolName );
				if ( outputs.isEmpty() ) {
				...
				const harvestTime = dropInfo.harvestTime;
				if ( harvestTime ) {
			...
			if ( monster.baseParameters.harvestPool ) {
				...
				const poopType = monster.baseParameters.canPoop || '1';
	 ...
	 */
	loadBees() {
		const beeConf = AssetDatabase.getData( 'bees/beeData.config' );
		for ( var [ beeType, subtypes ] of Object.entries( beeConf.stats ) ) {
			subtypes.forEach( ( info ) => {
				...
				const outputs = new RecipeSide();
				for ( const [ itemCode, infrequency ] of Object.entries( info.production ) ) {
	 ...
	 */
	loadIncubator() {
		const incubatorConf = AssetDatabase.getData( 'objects/crafting/eggstra/eggincubator/eggincubator.object' );
		for ( const [ eggItemCode, hatchResults ] of Object.entries( incubatorConf.incubation ) ) {
			const [ monsterCode, secondsToHatch, successRate ] = hatchResults;

			const quantityAttributes = { secondsToCraft: secondsToHatch };
			if ( successRate < 1 ) {
	...
	loadMonsterEvolutions() {
		MonsterDatabase.forEach( ( monster ) => {
			const parentType = monster.baseParameters.evolveType;
			if ( parentType ) {
	...
	loadBiomeBlocks() {
		BiomeDatabase.forEach( ( biome ) => {
			const biomeCode = biome.biomeCode;
			const outputs = new RecipeSide();
			for ( const materialName of [ biome.mainBlock ].concat( biome.subBlocks || [] ) ) {
				if ( !materialName ) {
					continue;
				}

				const material = MaterialDatabase.findByName( materialName );
				if ( !material ) {
					util.log( '[error] Biome ' + biomeCode + ' refers to unknown block: ' + materialName );
					continue;
				}

				const itemCode = material.itemDrop;
				if ( !itemCode ) {
	...
	loadBiomeMonsters() {
		BiomeDatabase.forEach( ( biome ) => {
			const spawnProfile = biome.spawnProfile;
			...
			for ( const spawnGroup of spawnProfile.groups ) {
				const pool = spawnGroup.pool;
				...
				const outputs = new RecipeSide();
				const optionsCount = pool.length;
				let selectedCount = spawnGroup.select;
				...
				for ( let [ individualChance, spawnType ] of util.normalizeWeights( pool ) ) {
					...
					let otherMonstersChance = 1 - individualChance;
					let remainingOtherMonsters = optionsCount - 1;
					let chanceToNotBeSelected = 1;

					for ( let roll = 0; roll < selectedCount; roll++ ) {
						...
						const newSum = individualChance + otherMonstersChance;
						individualChance /= newSum;
						otherMonstersChance /= newSum;
					}

					const quantityAttributes = {};
					const chance = 1 - chanceToNotBeSelected;
					if ( chance < 1 ) {
	 ...
	 */
	loadBiomeFish() {
		const fishingConf = AssetDatabase.getData( 'scripts/fishing/fishingspawner.config' );

		let previousRarityLevel = 0;
		const rarityChanceMultipliers = fishingConf.rarities.map( function ( rarityInfo ) {
			const [ rarityLevel, rarity ] = rarityInfo;
			const chanceMultiplier = Math.min( rarityLevel, 1 ) - previousRarityLevel;
		...
		for ( const [ biomeCode, pools ] of Object.entries( fishingConf.pools ) ) {
			const inputs = new RecipeSide();
			inputs.addBiome( biomeCode, { subtype: 'fish' } );

			const outputs = new RecipeSide();

			for ( const [ chanceMultiplier, rarity ] of rarityChanceMultipliers ) {
				const options = pools[rarity];
				const chance = 100 * chanceMultiplier / options.length;

				for ( const fishingOption of options ) {
					const notes = [];
					...
					const quantityAttributes = { chance: chance };
					if ( notes.length ) {
	 ...
	 */
	loadBiomePlaceables() {
		const treasureChests = AssetDatabase.getData( 'treasure/default.treasurechests' );
		const ignoredSmashPools = new Set( config.ignoredSmashPools );
		...
		BiomeDatabase.forEach( ( biome ) => {
			...
			for ( const [ placeablesConf, subtype ] of configs ) {
				if ( !placeablesConf ) {
				...
				( placeablesConf.items || [] ).forEach( ( placeable ) => {
					...
					if ( placeable.type === 'object' ) {
						for ( const objectSet of placeable.objectSets ) {
							const outputs = new RecipeSide();

							for ( let [ chance, itemCode ] of util.normalizeWeights( objectSet.pool ) ) {
								const quantityAttributes = { chance: chance * 100 };
								...
								if ( !item && itemCode.match( /(^|_)wild/ ) ) {
									...
									const seedItemCode = itemCode.replace( /(^|_)wild/, '' );
								...
								if ( isSmashable ) {
									...
									( item.breakDropOptions || [] ).concat( item.smashDropOptions || [] ).forEach( ( dropOption ) => {
										for ( const [ itemCode2 ] of dropOption ) {
											itemsDroppedFromBreakableObjects.add( itemCode2 );
										}
									} );

									const smashPool = item.smashDropPool || item.breakDropPool;
									if ( smashPool && !ignoredSmashPools.has( smashPool ) ) {
										const smashOutputs = TreasurePoolDatabase.getPossibleOutputs( smashPool );
								...
								const isHarvestable = item.stages && item.stages.some( ( stage ) => stage.harvestPool );
						...
						for ( const treasureBoxType of placeable.treasureBoxSets ) {
							for ( const treasureConf of treasureChests[treasureBoxType] ) {
								const outputs = new RecipeSide();
						...
						const outputs = new RecipeSide();
						for ( const stemType of placeable.treeStemList ) {
							outputs.addSaplingPart( stemType, false );
						}

						for ( const foliageType of placeable.treeFoliageList ) {
	...
	loadTerraformers() {
		ItemDatabase.forEach( ( itemCode, item ) => {
			const biomeCode = item.terraformBiome;
			if ( biomeCode ) {
	...
	loadBuildingUpgrades() {
		ItemDatabase.forEach( ( itemCode, item ) => {
			...
			const stages = [ item ].concat( item.upgradedStations );
			for ( let i = 0; i < stages.length - 1; i++ ) {
				const prevStage = stages[i];
				const nextStage = stages[i + 1];

				const upgradeMaterials = prevStage.interactData.upgradeMaterials;
	...
	loadItemUpgrades() {
		ItemDatabase.forEach( ( itemCode, item ) => {
			...
			const stages = [ item ].concat( item.upgradedItems );
			for ( let i = 0; i < stages.length - 1; i++ ) {
	...
	loadQuantumFuels() {
		[
			...
			const quantumConf = AssetDatabase.getData( pathToAsset );
			const maxHeatPower = Math.max( ...quantumConf.heat.map( ( heatLevel ) => heatLevel.power ) ),
				stationName = RemoveBadSymbols.fromName( quantumConf.shortdescription );

			for ( const [ itemCode, fuelPower ] of Object.entries( quantumConf.acceptablefuel ) ) {
				const inputs = new RecipeSide();
	...
	loadCombustionFuels() {
		[
			...
			const combustionConf = AssetDatabase.getData( pathToAsset );
			const maxHeatPower = Math.max( ...combustionConf.heat.map( ( heatLevel ) => heatLevel.power ) ),
				stationName = RemoveBadSymbols.fromName( combustionConf.shortdescription );

			for ( const [ itemCode, fuelDuration ] of Object.entries( combustionConf.acceptablefuel ) ) {
				const inputs = new RecipeSide();
				inputs.addItem( itemCode, { count: 1 } );
	 ...
	 */
	loadFissionFuels() {
		const fissionConf = AssetDatabase.getData( 'objects/power/isn_fissionreactornew/isn_fissionreactornew.object' );
		const toxicWasteCount = 1 + 0.01 * fissionConf.bonusWasteChance;

		for ( const [ itemCode, fuelOptions ] of Object.entries( fissionConf.fuels ) ) {
			const inputs = new RecipeSide();
			inputs.addItem( itemCode, { count: 1 } );
	 ...
	 */
	loadHydraulicFuels() {
		const hydraulicConf = AssetDatabase.getData( 'objects/power/hydraulicdynamo/hydraulicdynamo.object' );

		for ( const [ itemCode, fuelOptions ] of Object.entries( hydraulicConf.fuels ) ) {
			const inputs = new RecipeSide();
			inputs.addItem( itemCode, { count: 1 } );
	...
	loadFusionFuels() {
		[
			...
			const fusionConf = AssetDatabase.getData( pathToAsset ),
				stationName = RemoveBadSymbols.fromName( fusionConf.shortdescription );

			for ( const [ itemCode, fuelOptions ] of Object.entries( fusionConf.fuels ) ) {
				const inputs = new RecipeSide();
				inputs.addItem( itemCode, { count: 1 } );
	...
	loadTreasurePoolsFromOtherRecipes() {
		...
		for ( const recipe of this.knownRecipes ) {
			for ( const mentionedPoolName of recipe.outputs.getPoolNames() ) {
	...
	add( station, inputs, outputs, errorContext ) {
		const recipeObject = new Recipe( station, inputs, outputs );
	...
	addNativeCraftingRecipe( loadedData, filename ) {
		const station = CraftingStationDatabase.findByGroups( loadedData.groups );
		if ( !station ) {
			// This recipe can't be crafted anywhere.
			return;
		}

		const inputs = RecipeSide.newFromCraftingInput( loadedData.input );
		if ( loadedData.currencyInputs ) {
			// Used by Terraforge recipes, Teleporters, some EPP augments, etc.
			for ( const [ itemCode, count ] of Object.entries( loadedData.currencyInputs ) ) {
				inputs.addItem( itemCode, { count: count } );
			}
		}

		const outputs = RecipeSide.newFromCraftingInput( loadedData.output );
	...
	listMentionedItemCodes() {
		...
		this.knownRecipes.forEach( ( recipe ) => {
			const inputItems = recipe.inputs.getItemCodes(),
				outputItems = recipe.outputs.getItemCodes();

			for ( const mentionedItem of inputItems.concat( outputItems ) ) {
		...
		for ( const stationName of seenStationNames ) {
			const stationItemCode = ItemDatabase.findCodeByPageName( stationName );
			if ( stationItemCode ) {
