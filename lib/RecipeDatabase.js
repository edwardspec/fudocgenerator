	...
	loadMonsterDrops() {
		...
		MonsterDatabase.forEach( ( monster ) => {
			var pools = [
				...
				{ station: 'Bug Net', poolName: monster.getBugNetPool() }
			];
			...
			for ( var dropInfo of pools ) {
				...
				// FIXME: find a cleaner way to avoid monster->item conversion (done in RecipeComponent.newMonster()).
				if ( inputs.getAllCodes()[0] === outputs.getAllCodes()[0] ) {
					continue;
				}
			...
			( param.evolutions || [] ).forEach( ( pathToAsset ) => {
				...
				for ( var requirement of Object.values( evoConf.requiredResources ) ) {
					switch ( requirement.type ) {
						...
						case 'liquid':
							// TODO: implement RecipeSide.addLiquid() to avoid code duplication with loadLiquidCollector().
							var liquidId = requirement.id;
							var liquid = LiquidDatabase.find( liquidId );
							if ( !liquid ) {
								util.log( '[error] Unknown liquid in monster evolution: ' + pathToAsset + ': ' + liquidId );
								return;
							}

							inputs.addItem( liquid.itemDrop, { count: requirement.amount } );
							break;
