...
class RecipeDatabase {
	...
	loadStationGoods() {
		...
		for ( var goodsOption of stationConf.goods ) {
			let { baseAmount, abundance, lack } = goodsOption;

			let inputs = new RecipeSide();
			inputs.addItem( 'money', { count: goodsOption.basePrice } );
			inputs.addAvailableAmount( baseAmount * normalMultiplier );

			if ( abundance ) {
				inputs.addAvailableAmount( Math.round( baseAmount * abundanceMultiplier ), abundance );
			}
			if ( lack ) {
				inputs.addAvailableAmount( Math.round( baseAmount * lackMultiplier ), lack );
			}
