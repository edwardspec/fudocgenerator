	...
	loadBiomeMonsters() {
		BiomeDatabase.forEach( ( biome ) => {
			...
			for ( var spawnGroup of spawnProfile.groups ) {
				...
				// Pool has spawn types (not monsters) and can have subpools.
				// Let's convert it into a flat pool like [ [ 0.2, "monster1" ], [ 0.8, "monster2" ] ].
				var weightedMonsters = [];
				for ( var [ weight, spawnTypeName ] of util.normalizeWeights( pool ) ) {
					...
					weightedMonsters.push( [ weight, spawnType.monsterType ] );
				}

				// Some of "monsterType" values could have been subpools, so we must flatten the array.
				weightedMonsters = util.flattenWeightedPool( weightedMonsters );

				var outputs = new RecipeSide();
				var optionsCount = weightedMonsters.length;
				...
				for ( var [ individualChance, monsterType ] of util.normalizeWeights( weightedMonsters ) ) {
					...
					outputs.addMonster( monsterType, quantityAttributes );
				}
