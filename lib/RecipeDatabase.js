...
class RecipeDatabase {
	...
	/**
	 * Load all recipes of Honey Jarrer and Honey Jarring Machine.
	 */
	loadHoneyJarrer() {
	...
	loadIncubator() {
		...
		for ( var [ eggItemCode, hatchResults ] of Object.entries( incubatorConf.incubation ) ) {
			var [ monsterCode, secondsToHatch, successRate ] = hatchResults,
				monster = MonsterDatabase.find( monsterCode );

			if ( !monster ) {
				util.log( '[warning] Unknown monster in Incubator recipes: ' + monsterCode );
				continue;
			}
			...
			var outputs = new RecipeSide();
			outputs.addPseudoItem( '[[' + monster.displayName + ']]', quantityAttributes );
