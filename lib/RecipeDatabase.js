		...
		ItemDatabase.forEach( ( itemCode, data ) => {
			...
			for ( var [ stageNumber, stage ] of Object.entries( data.stages ) ) {
				...
				if ( !poolName ) {
					...
					inputs.addPseudoItem( "''Time: " + initialTime[0] + '-' + initialTime[1] + 's' +
						( isPerennial ? ' (first harvest)' : '' ) + "''" );

					if ( isPerennial ) {
						// When perennial crops are harvested, they revert to one of their earlier stages.
						var nextStage = stage.resetToStage;
						var regrowthTime = [
							util.sum( stageMinDurations.slice( nextStage ) ),
							util.sum( stageMaxDurations.slice( nextStage ) )
						];

						inputs.addPseudoItem( "''Time: " + regrowthTime[0] + '-' + regrowthTime[1] + "s (regrowth)''" );
					}
