	...
	loadStationShops() {
		...
		for ( var [ stationType, itemCodes ] of Object.entries( shopConf.potentialStock ) ) {
			var avgItems = ( stationType == 'generic' ) ? avgGenericItems : avgUniqueItems,
				...
				chanceComment = "''(" + util.trimFloatNumber( chanceOfEachItem, 1 ) + "% chance to be in stock)''";
			...
			itemCodes.forEach( ( itemCode ) => {
				...
				var inputs = new RecipeSide();
				inputs.addItem( 'money', { count: Math.ceil( ( soldItem.price || 1 ) * shopConf.initBuyMult ) } );
				inputs.addComment( chanceComment );
