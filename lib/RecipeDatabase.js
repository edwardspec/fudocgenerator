...
class RecipeDatabase {
			...
			for ( var [ placeablesConf, subtype ] of configs ) {
				...
				( placeablesConf.items || [] ).forEach( ( placeable ) => {
					...
					if ( placeable.type === 'object' ) {
						...
						// TODO: handle other types of placeables, most importantly "tree".
	...
	loadTreasurePoolsFromOtherRecipes() {
		...
		for ( var recipe of this.knownRecipes ) {
			if ( recipe.inputs.getAllCodes().join( '' ).startsWith( 'biome:' ) ) {
				// This is temporary, TODO: remove this after the Biome pages are actually created in the wiki.
				// Can't do it before, or many TreasurePool pages will be created for pools that are not obtainable
				// in any other way than from the biome.
				continue;
			}
