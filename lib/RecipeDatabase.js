...
class RecipeDatabase {
	...
	loadHarvestableItems() {
		ItemDatabase.forEach( ( itemCode, data ) => {
			...
			for ( var [ stageNumber, stage ] of Object.entries( data.stages ) ) {
				...
				if ( poolName ) {
					...
					// When perennial crops are harvested, they revert to one of their ealier stages.
					// Non-perennial crops start growing from stage 0 ("just planted").
					var nextStage = stage.resetToStage || 0;

					// Calculate time until next harvest.
					var minDuration = 0,
						maxDuration = 0;

					for ( var i = nextStage; i < stageNumber; i ++ ) {
						minDuration += stageMinDurations[i];
						maxDuration += stageMaxDurations[i];
					}

					inputs.addPseudoItem( "''(" + minDuration + '-' + maxDuration + "s)''" );
