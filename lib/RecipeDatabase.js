...
class RecipeDatabase {
	...
	loadTreasurePoolsFromOtherRecipes() {
		var seenPools = new Set();
		for ( var recipe of this.knownRecipes ) {
			for ( var mentionedPool of recipe.outputs.getPoolCodes() ) {
				seenPools.add( mentionedPool );
			}
		}

		for ( var poolName of [...seenPools].sort() ) {
			var inputs = new RecipeSide();
			...
			// TODO: "outputs" might contain pools that weren't in seenPools,
			// we must add those too recursively.
		}
	}
