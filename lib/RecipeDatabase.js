...
const { Recipe, RecipeSide, CraftingStationDatabase, AssetDatabase,
	...
	BiomeDatabase, MaterialDatabase, SpawnTypeDatabase,
	config, util } = require( '.' );
	...
	loadBiomeMonsters() {
		BiomeDatabase.forEach( ( biome ) => {
			...
			for ( var spawnGroup of spawnProfile.groups ) {
				...
				} else if ( optionsCount !== selectedCount ) {
					outputs.addComment( 'Any ' + selectedCount + ' of the following:' );
				}

				if ( pool[0][1] !== 'bee_honey' || selectedCount != 2 || optionsCount != 4 ) {
					continue;
				}

				console.log( pool );

				for ( var [ individualChance, spawnTypeName ] of util.normalizeWeights( pool ) ) {
					...
					// so the resulting chances are higher.
					// NOTE: this is still NOT an exact result, but a crude approximation,
					...
					var otherMonstersChance = 1 - individualChance,
						chanceReductionPerThrow = otherMonstersChance / ( optionsCount - 1 );

					console.log( "chanceReductionPerThrow=" + chanceReductionPerThrow );

					var chanceToNotBeSelected = 1;
					for ( var roll = 0; roll < selectedCount; roll ++ ) {
						console.log( "chanceToNotBeSelected (" + chanceToNotBeSelected + ") *= " + otherMonstersChance );

						chanceToNotBeSelected *= otherMonstersChance;
						otherMonstersChance -= chanceReductionPerThrow;

						console.log( "new chanceToNotBeSelected=" + chanceToNotBeSelected + ", new otherMonstersChance=" + otherMonstersChance );
					...
					console.log( spawnTypeName, chance );
					require('process').exit();

					var spawnType = SpawnTypeDatabase.find( spawnTypeName );
					if ( !spawnType ) {
						util.log( '[error] Unknown spawnType=' + spawnTypeName + ' in biome: ' + biome.biomeCode );
						return;
					}
					...
					outputs.addMonster( spawnType.monsterType, quantityAttributes );
				}
