...
class RecipeDatabase {
	...
	load() {
		...
		// Which music plays in which biomes.
		this.loadBiomeMusic();
		...
		for ( var [ buildingName, buildingConf ] of Object.entries( smelterBuildings ) ) {
			for ( var [ inputItem, outputItem ] of Object.entries( buildingConf.inputsToOutputs ) ) {
				...
				for ( var [ bonusOutputItem, percent ] of Object.entries( bonusOutputs ) ) {
					outputs.addItem( bonusOutputItem, { chance: percent * buildingConf.fu_extraProductionChance } );
				}
	...
	/**
	 * Add "biome -> possible background music" recipes.
	 */
	loadBiomeMusic() {
		BiomeDatabase.forEach( ( biome ) => {
			var biomeCode = biome.biomeCode;
			...
			for ( var [ daytime, musicConf ] of Object.entries( biome.musicTrack || {} ) ) {
				...
				var inputs = new RecipeSide();
				inputs.addBiome( biomeCode, daytime );

				this.add( 'Biome music', inputs, outputs );
			}
		} );
	}
