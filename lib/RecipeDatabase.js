		...
		BiomeDatabase.forEach( ( biome ) => {
			...
			for ( var spawnGroup of spawnProfile.groups ) {
				...
				for ( var [ individualChance, spawnTypeName ] of util.normalizeWeights( pool ) ) {
					...
					// so the resulting chances are higher. Let's calculate them here.

					// FIXME: not yet implemented. Until it is, we are using fallback value (individual chance only).
					var chance = individualChance;
