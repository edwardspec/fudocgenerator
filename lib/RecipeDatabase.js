				...
				( placeablesConf.items || [] ).forEach( ( placeable ) => {
					if ( placeable.type === 'object' ) {
						for ( var objectSet of placeable.objectSets ) {
							...
							for ( var [ chance, itemCode ] of util.normalizeWeights( objectSet.pool ) ) {
								...
								if ( !item && itemCode.match( /(^|_)wild/ ) ) {
									...
									var seedItem = ItemDatabase.find( seedItemCode );
								...
								if ( item.breakDropOptions && item.breakDropOptions.length > 0 ) {
									// Breakable objects are unobtainable by player, so we don't need articles about them.
									// TODO: there can probably be a separate Recipe for items that are dropped
									// when the breakable objects are smashed.
									util.log( '[info] Ignoring breakable object: ' + itemCode );
								} else {
						...
						// TODO: handle other types of placeables, most importantly "tree" and "treasureBox".
						// Types "grass" and "bush" are probably not as important (although a bush can drop things?),
						// and the only thing we can gather about "microdungeon" at this point is its machine-readable ID.
					}
				} );
			}
		} );
