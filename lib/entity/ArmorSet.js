...
const { CargoRow, PageNameRegistry, RemoveBadSymbols, config, util } = require( '..' );

const armorSetsWithoutSetBonus = new Set( config.armorSetsWithoutSetBonus );
...
class ArmorSet {
	...
	addItem( item, slot ) {
		...
		var setBonus = RemoveBadSymbols.fromDescription( item.description );
		var part = '*';

		var bonusPartMatch = setBonus.match( /Set Bonuses[^\n]*Part ([0-9].*)/ );
		if ( bonusPartMatch ) {
	...
	toCargoDatabase() {
		...
		if ( !armorSetsWithoutSetBonus.has( this.id ) ) {
			fields.setBonus = Object.values( this.bonus ).join( '\n----\n' );
		}

		var getPageNameWithoutLazyAllocation = ( item ) => {
