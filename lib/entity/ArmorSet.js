...
class ArmorSet {
	...
	toCargoDatabase() {
		...
		if ( this.head ) {
			...
			fields.headPage = this.head.map( getPageNameWithoutLazyAllocation );
		}
		if ( this.chest ) {
			fields.chest = this.chest.map( ( item ) => item.itemCode );
			fields.chestPage = this.chest.map( getPageNameWithoutLazyAllocation );
		}
		if ( this.legs ) {
			fields.legs = this.legs.map( ( item ) => item.itemCode );
			fields.legsPage = this.legs.map( getPageNameWithoutLazyAllocation );
		}
