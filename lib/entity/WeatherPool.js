...
class WeatherPool {
	...
	constructor( poolName, possibleWeathers ) {
		...
		for ( const [ weatherWeight, weatherCode ] of this.possibleWeathers ) {
			const weather = Query.findWeather( weatherCode );
			if ( !weather ) {
				util.log( '[warning] unknown weather "' + weatherCode + '" in weather pool "' + poolName + '".' );
				continue;
			}

			const averageDuration = weatherWeight * 0.5 * ( weather.duration[0] + weather.duration[1] );
	...
	toWikitext() {
		...
		const normalizedWeatherChances = util.normalizeWeights( this.possibleWeathers )
			.sort( ( a, b ) => b[0] - a[0] );

		let wikitext = '';
		for ( const [ chance, weatherCode ] of normalizedWeatherChances ) {
			wikitext += '\n* ';

			const weatherInfo = Query.getWeatherNameAndIcon( weatherCode );
