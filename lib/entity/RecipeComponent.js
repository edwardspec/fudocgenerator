...
class RecipeComponent {
	...
	static newMonster( monsterCode, quantityAttributes = {} ) {
		...
		var critter = Query.findCritter( monsterCode );
		if ( critter ) {
	...
	static newTenant( tenantCode ) {
		var self = new RecipeComponent( tenantCode );
		self.isTenant = true;
	 ...
	 * @return {RecipeComponent}
	 */
	static newPseudoItem( displayName, quantityAttributes = {}, overrideItemId = null ) {
		var self = new RecipeComponent( '', quantityAttributes );
		self.isPseudo = true;
		self.id = ':' + ( overrideItemId || ++pseudoItemCounter );
	...
	attemptMerge( anotherComponent ) {
		...
		for ( let [ key, value ] of Object.entries( anotherComponent.quantity ) ) {
	...
	getDisplayNameUncached() {
		...
		if ( this.isTenant ) {
			var tenant = TenantDatabase.find( this.code );
			if ( !tenant ) {
	...
	toWikitext( renderParameters ) {
		...
		var subtype = amount.subtype;
		if ( Array.isArray( subtype ) ) {
			subtype = subtype.filter( ( x ) => {
				// Omit "peaceful" note for nameless critters and bees (they are all peaceful).
				return x && !( x === 'peaceful' && ( this.isCritter || this.code.endsWith( '_queen' ) ) );
			} ).join( ', ' );
		}
		...
		if ( amount.averageCount ) {
			...
			// If only 1 monster out of 5 drops 2 of some item, then this value will be 2/5=0.4.
	...
	getSubComponents() {
		if ( this.isItem && this.code === 'sapling' ) {
			var itemParams = this.quantity.parameters;
			if ( itemParams ) {
				return [
