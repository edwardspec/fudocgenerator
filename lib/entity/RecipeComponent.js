...
const { AssetDatabase, TenantDatabase, QuestDatabase, Query, config, util } = require( '..' );
...
class RecipeComponent {
	...
	static newItem( itemCode, quantityAttributes = {} ) {
		...
		const self = new RecipeComponent( itemCode, quantityAttributes );
		self.isItem = true;
		self.id = itemCode;
		return self;
	}
	...
	static newMonster( monsterCode, quantityAttributes = {} ) {
		const replacementCode = config.identicalMonsters[monsterCode];
		if ( replacementCode ) {
		...
		const self = new RecipeComponent( monsterCode, quantityAttributes );

		const critter = Query.findCritter( monsterCode );
		if ( critter ) {
			const bugNetPoolName = critter.getBugNetPool();
			if ( bugNetPoolName ) {
				const pool = Query.findTreasurePool( bugNetPoolName );
				if ( pool ) {
					const lootItems = pool.getPossibleOutputs().getItemCodes();
					if ( lootItems.length ) {
						// Instead of capturable bug (monster) we can show the "caught bug" placeable item.
						return RecipeComponent.newItem( lootItems[0], quantityAttributes );
					}
				}
			}
	 ...
	 * @return {RecipeComponent}
	 */
	static newBiome( biomeCode, quantityAttributes = {} ) {
		const self = new RecipeComponent( biomeCode, quantityAttributes );
		self.isBiome = true;
	...
	static newMusicTrack( assetPath ) {
		const self = new RecipeComponent( assetPath );
	...
	/**
	 * Make 1 tenant pseudo-item (something that has a valid tenant ID) into RecipeComponent.
	 *
	 * @param {string} tenantCode
	 * @return {RecipeComponent}
	 */
	static newTenant( tenantCode ) {
		const self = new RecipeComponent( tenantCode );
		self.isTenant = true;
		self.id = 'tenant:' + tenantCode;
		return self;
	}
	...
	static newQuest( questCode ) {
		const self = new RecipeComponent( questCode );
		self.isQuest = true;
	...
	static newSaplingPart( name, isFoliage ) {
		const self = new RecipeComponent( name );
	...
	static newPool( poolName, weight ) {
		...
		const self = new RecipeComponent( poolName, quantityAttributes );
		self.isPool = true;
	...
	static newPseudoItem( displayName, quantityAttributes = {} ) {
		const self = new RecipeComponent( '', quantityAttributes );
		self.isPseudo = true;
	...
	attemptMerge( anotherComponent ) {
		...
		const keys = Object.keys( anotherComponent.quantity ).sort(),
		...
		const unmergeableKeys = keys.filter( ( key ) => !mergeableQuantities.includes( key ) );
		...
		for ( const [ key, value ] of Object.entries( anotherComponent.quantity ) ) {
	 ...
	 */
	isValid() {
		for ( const [ attr, value ] of Object.entries( this.quantity ) ) {
	...
	isValidAttribute( attr, value ) {
		...
		switch ( attr ) {
			...
			case 'rarity':
				...
				if ( ![ 'common', 'normal', 'uncommon', 'rare', 'rarest' ].includes( rarity ) ) {
					// Unknown rarity level.
					return false;
				}

				if ( Number.isNaN( Number( divisor ) ) ) {
					// Not a valid number.
					return false;
				}
				return true;
	...
	getDisplayNameUncached() {
		if ( this.isMonster ) {
			const monster = Query.findMonster( this.code );
			if ( !monster ) {
				if ( this.isCritter ) {
					// Non-capturable decorative critter, we don't gather any information about them.
					return 'Critter: ' + this.code;
				}

				util.warnAboutUnknownMonster( this.code );
				return 'Unknown monster: ' + this.code;
			}
		...
		if ( this.isQuest ) {
			...
			const quest = QuestDatabase.find( this.code );
			if ( !quest ) {
		...
		if ( this.isTenant ) {
			const tenant = TenantDatabase.find( this.code );
			if ( !tenant ) {
				util.log( '[warn] Unknown tenant in the recipe: ' + this.code );
				return '';
			}

			return tenant.getWikiPageLink();
		}
		...
		if ( this.isPool ) {
			...
			const [ poolCode, isSubpool ] = this.code.split( ':' );
			...
			const pool = Query.findTreasurePool( poolCode );
			if ( !pool ) {
			...
			if ( isSubpool ) {
				...
				const subpool = Query.findTreasurePool( this.code );
		...
		if ( this.isBiome ) {
			const biome = Query.findBiome( this.code );
			if ( !biome ) {
		...
		if ( this.isMusicTrack ) {
			...
			const asset = AssetDatabase.get( this.code );
			if ( !asset || asset.vanilla ) {
				return this.label;
			}

			const url = config.githubLink + '/blob/master' + this.code;
			return '[' + url + ' ' + this.label + ']';
		}

		if ( this.isSaplingPart ) {
			const saplingPart = this.isFoliage ? Query.findFoliage( this.code ) : Query.findStem( this.code );
			if ( !saplingPart ) {
		...
		// Normal item.
		const item = Query.findItem( this.code );
		if ( !item ) {
	...
	toWikitext( renderParameters ) {
		...
		if ( amount.rarity ) {
			...
			const [ rarity, chanceDivisor ] = amount.rarity;
		...
		if ( amount.planets ) {
			...
			const allPlanetNames = amount.planets.map( ( thisPlanetCode ) => Query.getPlanetName( thisPlanetCode ) ),
		...
		const itemParams = amount.parameters;
		if ( itemParams && this.isItem ) {
			...
			if ( this.code === 'sapling' ) {
				const saplingName = Query.getSaplingName(
		...
		if ( amount.chanceToSell ) {
			// Round to 1 digit.
			wikitext += "\n''(" + util.trimFloatNumber( amount.chanceToSell, 1 ) + "% chance to be in stock)''";
		}

		if ( ( amount.isComment || amount.chanceToSell ) && !amount.noLineBreak ) {
	...
	/**
	 * Get an array of components that should be included into recipes alongside this component.
	 * For example, a Sapling item component may include stem/foliage as subcomponents.
	 *
	 * @return {RecipeComponent[]}
	 */
	getSubComponents() {
		if ( this.isItem && this.code === 'sapling' ) {
			const itemParams = this.quantity.parameters;
			if ( itemParams ) {
				return [
					RecipeComponent.newSaplingPart( itemParams.foliageName, true ),
					RecipeComponent.newSaplingPart( itemParams.stemName, false )
				];
			}
		}
