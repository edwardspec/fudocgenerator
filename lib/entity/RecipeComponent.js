...
const { QuestDatabase, Query, config, util } = require( '..' );
...
class RecipeComponent {
	...
	static newBiome( biomeCode, extraComment ) {
		var self = new RecipeComponent( biomeCode, { subtype: extraComment } );
		self.isBiome = true;
	...
	/**
	 * Make 1 modular tree stem (something that has a valid stem ID) into RecipeComponent.
	 *
	 * @param {string} stemName
	 * @return {RecipeComponent}
	 */
	static newStem( stemName ) {
		var self = new RecipeComponent( stemName );
		self.isStem = true;
		self.id = 'stem:' + stemName;
		return self;
	}

	/**
	 * Make 1 modular tree foliage (something that has a valid foliage ID) into RecipeComponent.
	 *
	 * @param {string} foliageName
	 * @return {RecipeComponent}
	 */
	static newFoliage( foliageName ) {
		var self = new RecipeComponent( foliageName );
		self.isFoliage = true;
		self.id = 'foliage:' + foliageName;
		return self;
	}
	...
	getDisplayNameUncached() {
		...
		if ( this.isPool ) {
			...
			var pool = Query.findTreasurePool( this.code );
			if ( !pool ) {
				util.log( '[warn] Unknown TreasurePool in the recipe: ' + this.code );
				return '';
			}

			return pool.getWikiPageLink();
		}
		...
		if ( this.isStem ) {
			var stem = Query.findStem( this.code );
			if ( !stem ) {
				util.log( '[warn] Unknown modularstem in the recipe: ' + this.code );
				return '';
			}

			// Link to the article about this stem.
			return '[[' + stem.wikiPageName + '|Stem: ' + stem.name + ']]';
		}

		if ( this.isFoliage ) {
			var foliage = Query.findFoliage( this.code );
			if ( !foliage ) {
				util.log( '[warn] Unknown modularfoliage in the recipe: ' + this.code );
				return '';
			}

			// Link to the article about this foliage.
			return '[[' + foliage.wikiPageName + '|Foliage: ' + foliage.name + ']]';
		}
