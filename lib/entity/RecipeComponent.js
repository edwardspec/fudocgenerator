...
class RecipeComponent {
	...
	static newItem( itemCode, quantityAttributes = {} ) {
		...
		var blueprintMatch = itemCode.match( /^(.*)-recipe/ );
		if ( blueprintMatch ) {
			itemCode = blueprintMatch[1];
			...
			if ( this.isMonster ) {
				...
				if ( !monster ) {
					if ( this.code.endsWith( 'critter' ) ) {
						...
						// NOTE: this is insufficient, because some critters have codes like "fusnowball".
						// TODO: Handle this in MonsterDatabase.
				...
				this.displayName = pool.getWikiPageLink();
			} else if ( this.isBiome ) {
				...
				this.displayName = biome.getWikiPageLink( 'Biome: ' );
			} else {
				// Normal item.
