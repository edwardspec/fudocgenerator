...
// Workaround for bug in Cargo+MW 1.36, which makes all integer/boolean fields mandatory.
...
class CargoRow {
	...
	constructor( cargoTable, fields, extraOptions = {} ) {
		...
		// Apply default values. This is only needed to workaround a bug in Cargo+1.36
		// (which makes all Integer/Boolean field mandatory).
		for ( var [ fieldName, defaultValue ] of Object.entries( mandatoryFields[this.table] || {} ) ) {
