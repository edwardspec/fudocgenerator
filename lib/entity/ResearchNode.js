...
const { ItemDatabase, RecipeSide, CargoRow, RemoveBadSymbols, util } = require( '..' );
...
class ResearchNode {
	...
	constructor( rawData, nodeId, treeName, nodeName, description ) {
		...
		this.name = RemoveBadSymbols.fromName( nodeName );
		this.description = RemoveBadSymbols.fromDescription( description );
		...
		( rawData.price || [] ).forEach( ( priceComponent ) => {
			const [ itemCode, count ] = priceComponent;
	...
	toCargoDatabase() {
		...
		const priceWikitext = this.price.toWikitext().replace( /\|/g, '{{!}}' );

		const unlocksWikitext = this.unlocks.map( ( itemCode ) => {
		...
		const { ResearchTreeDatabase } = require( '..' ),
