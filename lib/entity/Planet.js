...
class Planet {
	...
	toCargoDatabase() {
		...
		const colorLevels = this.getDayLightColors().map( ( light ) => light.getLevel() ).sort( util.compareNum );
		...
		const windLevels = this.getWindPowerOptions().sort( util.compareNum );
	...
	getRegionCodes( regionCodes ) {
		const tierAdjustedCodes = new Set();
		for ( const regionCode of regionCodes ) {
			const region = Query.findRegion( regionCode );
			for ( const code of region.getCodesForTier( this.minTier, this.maxTier ) ) {
	 ...
	 */
	getDayLightColors() {
		const surfacePrimaryRegions = this.layers.surface.primaryRegion || [];
