...
class Planet {
	...
	toCargoDatabase() {
		...
		for ( var [ layerName, layerInfo ] of Object.entries( this.layers ) ) {
			var fieldsForLayer = {
				...
				layer: layerName,
				primaryRegion: this.addSubRegions( layerInfo.primaryRegion || [] ),
				secondaryRegions: this.addSubRegions( layerInfo.secondaryRegions || [] )
			};
	 ...
	 * return an array of these regions codes PLUS region codes of any subregions of these regions.
	 * For example, "volcanic" region can have "geode" subregion, so the string "geode" will also
	 * be in the returned array.
	 *
	 * @param {string[]} regionCodes
	 * @return {string[]}
	 */
	addSubRegions( regionCodes ) {
		var subregions = [];
		for ( var regionCode of regionCodes ) {
			var region = Query.findRegion( regionCode );
			for ( var code of ( region.subRegion || [] ) ) {
				subregions.push( code );
			}
		}

		return [...new Set( regionCodes.concat( subregions ) )];
	}
}
