...
class Planet {
	...
	toCargoDatabase() {
		...
		var { PlanetDatabase } = require( '..' );
		for ( var [ layerName, layerInfo ] of Object.entries( this.layers ) ) {
			// TODO: move this into a function - same is needed for layerInfo.secondaryRegions.
			for ( var regionName of ( layerInfo.primaryRegion || [] ) ) {
				var region = PlanetDatabase.knownRegions[regionName];

				wikitext = '{{#cargo_store:_table = layer\n';

				wikitext += '|planet=' + this.id + '\n';
				wikitext += '|layer=' + layerName + '\n';
				wikitext += '|biomeRole=main\n';
				wikitext += '|biome=' + region.biome[0][1][0] + '\n';

				if ( region.oceanLiquid ) {
					wikitext += '|oceanLiquid=' + region.oceanLiquid + '\n';
				}
				if ( region.caveLiquid ) {
					wikitext += '|caveLiquid=' + region.caveLiquid + '\n';
				}

				wikitext += '}} ';
			}

			// TODO: layerInfo.dungeons?
		}
