...
class Planet {
	...
	toCargoDatabase() {
		...
		// Calculate best/worst light color (for knowing how viable is Solar Panel on this planet).
		var minDayLight = 1, maxDayLight = 0;
		for ( var lightColor of this.getDayLightColors() ) {
			var level = lightColor.getLevel();

			if ( minDayLight > level ) {
				minDayLight = level;
			}

			if ( maxDayLight < level ) {
				maxDayLight = level;
			}
		}
		fields.minDayLight = minDayLight;
		fields.maxDayLight = maxDayLight;

		// TODO: add a wikitext field with actual CSS colors and detailed list of chances
		// to have each light level (e.g. "0.6 light (75%), 0.2 light (25%)").
