...
class Item {
	...
	constructor( asset, overrideId = '' ) {
		...
		if ( this.interactData ) {
			...
			if ( typeof ( interactData.config ) === 'string' ) {
				...
				let configAsset = AssetDatabase.get( interactData.config );
				if ( !configAsset ) {
					...
					for ( let [ key, value ] of Object.entries( configAsset.data ) ) {
						if ( !interactData[key] ) {
		...
		this.isConsumable = this.asset.filename.endsWith( '.consumable' );
		...
		if ( this.upgradedStations ) {
			...
			let firstStage = this.upgradedStations.shift();
			if ( firstStage ) {
				for ( let [ key, value ] of Object.entries( firstStage ) ) {
	...
	makeSubItem( stageInfo, pseudoId ) {
		...
		// (the only time when shortdescription is shown is if when the user moves the building into inventory)
	...
	get metadata() {
		...
		if ( agingScripts && agingScripts.includes( '/scripts/items/rotting.lua' ) ) {
			// Gather "how many minutes before this food rots".
		...
		if ( this.baseShieldHealth && this.perfectBlockTime ) {
			var shieldHealthMultiplier = this.hasBuildscript ?
		...
		if ( this.materialId ) {
			...
			// Effects when stepping on this block, if any.
			...
			// Effects when swimming in this liquid, if any.
			var statusEffects = ( liquid.statusEffects || [] ).filter( ( x ) => x !== 'swimming' && x !== 'biomeairless' );
			if ( statusEffects.length ) {
				// Item can't be both material and liquid, so we can use the same field as for tile effects.
				metadata.tileEffects = statusEffects;
			}
		}
	...
	/**
	 * Get path to the image of placeable object (such as Extraction Lab or Wooden Crate), if any.
	 *
	...
	getPlacedImage() {
		...
		var placedObject = ( this.orientations || [] )[0];
		if ( !placedObject ) {
			return;
		}

		var placedImage = placedObject.dualImage || placedObject.image;
		if ( placedImage ) {
			return placedImage;
		}

		if ( !placedObject.imageLayers ) {
			return;
		}
