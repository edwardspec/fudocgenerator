...
class Item {
	...
	getMetadata() {
		...
		var [ , beeSpecies ] = this.itemCode.match( /bee_(.*)_queen/ ) || [];
		if ( beeSpecies ) {
			metadata.rivals = Query.getBeeRivals( beeSpecies ).join( ',' );

			for ( var [ subspecies, workTime ] of Object.entries( Query.getBeeWorkTimes( beeSpecies ) ) ) {
				metadata['workTime.' + subspecies] = workTime;
			}

			// TODO: add preferred biomes, but only after we can autocreate articles about them.
		}
