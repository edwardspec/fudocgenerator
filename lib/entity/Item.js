...
class Item {
	...
	static newFromCodex( asset ) {
		...
		data.shortdescription = util.cleanPageName( data.title.replace( //, '' ) )
			.replace( /[\r\n]+/, ' ' ) + ' (codex)';
	...
	toCargoDatabase() {
		...
		for ( var [ key, value ] of Object.entries( this.getMetadata() ) ) {
			...
			wikitext += '|id=' + this.itemCode;
			wikitext += '|prop=' + key;
			wikitext += '|value=' + util.escapeParameterOfCargoStore( value )
			wikitext += '}} ';
		}
		...
		if ( this.contentPages ) {
			wikitext += '{{#cargo_store:_table = codex_text\n';
			wikitext += '|id=' + this.itemCode;
			wikitext += '|text=' + util.escapeParameterOfCargoStore( this.contentPages.join( '\n----\n' ) )
			wikitext += '}} ';
		}
