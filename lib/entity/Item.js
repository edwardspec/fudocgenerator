...
const { LoadedAsset, Query, EntityWithPageName, FunctionsDatabase, MaterialDatabase,
		WeaponAbilityDatabase, AssetDatabase, CargoRow, RemoveBadSymbols, util } = require( '..' ),
...
class Item {
	...
	constructor( asset, overrideId = '' ) {
		...
		var upgradeStages = this.upgradeStages || [];
		if ( upgradeStages ) {
			// Upgradeable crafting station, e.g. Machining Table -> Auto-assembler
			this.isUpgradeableStation = true;
		} else if ( this.upgradeParameters ) {
			// Upgradeable weapon/tool, e.g. Fusion Cannon -> Blastmark.
			for ( let idx = 1, keySuffix = ''; ; idx ++, keySuffix = idx ) {
				var value = this['upgradeParameters' + keySuffix];
				if ( !value ) {
					break;
				}

				upgradeStages.push( value );
			}
		}
		...
		this.upgradedItems = upgradeStages.map( ( stageInfo, index ) => {
