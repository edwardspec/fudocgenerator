...
class Item {
	...
	constructor( asset, overrideId = '' ) {
		...
		if ( this.interactData ) {
			...
			if ( typeof ( interactData.config ) === 'string' ) {
				...
				if ( !configAsset ) {
					...
					for ( const [ key, value ] of Object.entries( configAsset.data ) ) {
						if ( !interactData[key] ) {
		...
		if ( this.upgradeParametersTricorder ) {
			...
			const value = this.upgradeParametersTricorder;
		...
		if ( this.upgradeParameters ) {
			...
			while ( true ) {
				const value = this['upgradeParameters' + keySuffix];
				if ( !value ) {
					break;
				}
				...
				const subItem = this.makeSubItem( value, this.itemCode + ':' + index );
		...
		if ( this.builder && this.builder.match(
			/\/(fubuild(armor|chakram|magnorb|unrandshield)|build(boomerang|fist|shield|unrand(weapon|shield)|whip)|(neb-)?buildbow)\.lua$/
		) ) {
		...
		( this.leveledStatusEffects || [] ).forEach( ( leveledBonus ) => {
			const stat = leveledBonus.stat;

			let multiplier = FunctionDatabase.calculate( leveledBonus.levelFunction, this.level || 1 );
			const value = ( leveledBonus.amount || 1 );
			...
			let scaledValue = util.trimFloatNumber( value * multiplier, 2 );
		...
		this.upgradedStations = ( this.upgradeStages || [] ).map( ( stageInfo, index ) => {
			const pseudoId = this.itemCode + ':' + ( this.startingUpgradeStage + index );
		...
		if ( this.upgradedStations ) {
			...
			const firstStage = this.upgradedStations.shift();
			if ( firstStage ) {
				for ( const [ key, value ] of Object.entries( firstStage ) ) {
		...
		if ( this.interactData && this.interactData.initialRecipeUnlocks ) {
			const blueprints = ( this.learnBlueprintsOnPickup || [] )
	...
	static newFromCodex( asset ) {
		...
		for ( const [ key, value ] of Object.entries( data.itemConfig || {} ) ) {
			data[key] = value;
		}
	...
	mergeAbility( inlineAbility, abilityType ) {
		...
		const result = inlineAbility || {};
		if ( !abilityType ) {
			return result;
		}

		const abilityConf = WeaponAbilityDatabase.find( abilityType );
		if ( !abilityConf ) {
			return result;
		}
		...
		const ability = abilityConf.ability || abilityConf;
	...
	makeSubItem( stageInfo, pseudoId ) {
		...
		const subitemData = {};
		for ( const dataSource of [ this, stageInfo, stageInfo.itemSpawnParameters || {} ] ) {
		...
		const interactData = subitemData.interactData || {};
		for ( const layout of [ interactData.paneLayout, interactData.paneLayoutOverride ] ) {
			if ( !layout ) {
				continue;
			}
			...
			if ( layout.lblTitle ) {
				...
				const shopName = ( layout.windowtitle.title || '' ).trim();
				if ( shopName ) {
		...
		const subAsset = LoadedAsset.newChildAsset( this.asset, subitemData );
	...
	toCargoDatabase() {
		...
		if ( this.contentPages ) {
			const codexText = RemoveBadSymbols.fromDescription( this.contentPages.join( '\n----\n' ) );
	...
	get metadata() {
		...
		if ( this.builder === '/items/buildscripts/buildfood.lua' ) {
			...
			const agingScripts = this.itemAgingScripts;
		...
		const whichAnimalsEat = Query.whichAnimalsEat( this.itemCode );
		if ( whichAnimalsEat ) {
		...
		const damageMultiplier = this.hasBuildscript ?
			FunctionDatabase.calculate( 'weaponDamageLevelMultiplier', this.level ) : 1;

		// Add damage, elemental type, etc. of left-click and right-click attacks.
		const projectile = this.projectileConfig || this.projectileParameters; // For thrown weapons
		...
		// Shield bonuses
		if ( this.baseShieldHealth && this.perfectBlockTime ) {
			const shieldHealthMultiplier = this.hasBuildscript ?
				FunctionDatabase.calculate( 'shieldLevelMultiplier', this.level ) : 1;

			metadata.shieldHealth = Math.round( this.baseShieldHealth * shieldHealthMultiplier );
			metadata.perfectBlockTime = this.perfectBlockTime;
		}
		...
		let lightColorRGB = this.lightColor;

		// Block hitpoints (for knowing "how well does this block resist damaging weather")
		if ( this.materialId ) {
			const material = MaterialDatabase.find( this.materialId );
			if ( material.health ) {
				metadata.blockHealth = material.health;
			}

			const tileEffects = Query.getTileEffects( material.materialName );
		...
		if ( this.liquid ) {
			const liquid = LiquidDatabase.findByName( this.liquid );
			lightColorRGB = liquid.radiantLight;
		}

		if ( lightColorRGB ) {
			// This object/block/liquid emits light.
			const light = new LightColor( ...lightColorRGB );
		...
		const mechFuel = Query.getMechFuelInfo( this.itemCode );
		if ( mechFuel ) {
