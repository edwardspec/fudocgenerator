...
const { Query, MaterialDatabase, WeaponAbilityDatabase, config, util } = require( '..' );
...
class Item {
	...
	getMetadata() {
		...
		var damageMultiplier = this.hasBuildscript ? ( 0.5 + tier / 2 ) : 1;
		...
		( this.leveledStatusEffects || [] ).forEach( ( leveledBonus ) => {
			...
			var multiplier = Query.getMultiplier( leveledBonus.levelFunction, tier );
