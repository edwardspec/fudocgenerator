...
const { Query, FunctionsDatabase, MaterialDatabase,
	WeaponAbilityDatabase, CargoRow, config, util } = require( '..' ),
...
class Item {
	...
	constructor( asset ) {
		...
		if ( this.builder && this.level !== undefined &&
			this.builder.match( /\/(fubuildarmor|buildunrandweapon|buildwhip|fubuildmagnorb|buildboomerang|fubuildchakram)\.lua$/ )
		) {
	...
	toCargoDatabase() {
		...
		// TODO: omit this field if 0. Might need changes in Scribunto module.
		fields.upgradeable = ( isUpgradeable ? 1 : 0 );
		...
		if ( this.learnBlueprintsOnPickup ) {
			...
			this.learnBlueprintsOnPickup.forEach( ( unlockedItemCode ) => {
				...
				fields.unlocks.push( unlockedItemCode );
			} );
		}

		var wikitext = ( new CargoRow( 'item', fields ) ).toWikitext();

		// Write key-value pairs of metadata into a separate Cargo table.
		for ( var [ key, value ] of Object.entries( this.getMetadata() ) ) {
			// FIXME: just make toCargoDatabase() return an array of CargoRow objects.
			wikitext += ( new CargoRow( 'item_metadata', {
				id: this.itemCode,
				prop: key,
				value: util.escapeParameterOfCargoStore( value )
			}, { compact: true } ) ).toWikitext();
		}
		...
		if ( this.contentPages ) {
			...
			wikitext += ( new CargoRow( 'codex_text', {
				id: this.itemCode,
	...
	getMetadata() {
		...
		for ( var [ stat, value ] of Object.entries( this.stats ) ) {
			if ( stat.match( /Resistance$/ ) ) {
				// Skip elementary resistances: they take too much space, should be in "armorset" table only.
				continue;
			}
