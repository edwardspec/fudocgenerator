...
const { Query, PageNameRegistry, FunctionsDatabase, MaterialDatabase,
...
class Item {
	...
	constructor( asset, overrideId = '' ) {
		...
		this.upgradedItems = ( this.upgradeStages || [] ).map( ( stageInfo, index ) => {
			...
			// TODO: move "create subAsset" logic into LoadedAsset class.
			var subAsset = {
	...
	/**
	 * Get PageName assigned to this Item by PageNameRegistry, or empty string if none was assigned.
	...
	toArticleText( wikiPageName ) {
		return "{{-start-}}\n'''" + wikiPageName + "'''\n" +
			'{{Automatic infobox item|' + this.itemCode + "}}<!-- Please don't delete this line -->\n" +
