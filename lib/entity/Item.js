...
const { Query, FunctionsDatabase, MaterialDatabase,
	WeaponAbilityDatabase, config, util } = require( '..' ),
...
class Item {
	...
	constructor( asset ) {
		...
		if ( this.builder && this.level !== undefined &&
			this.builder.match( /\/(fubuildarmor|buildunrandweapon|buildwhip|fubuildmagnorb|buildboomerang)\.lua$/ )
		) {
	...
	getMetadata() {
		...
		var damageMultiplier = this.hasBuildscript ?
			FunctionsDatabase.calculate( 'weaponDamageLevelMultiplier', tier ) : 1;
		...
		// These are mainly armor bonuses to protection, health, energy and damage.
		( this.leveledStatusEffects || [] ).forEach( ( leveledBonus ) => {
			var stat = leveledBonus.stat;
			if ( stat.match( /Resistance$/ ) ) {
				// Skip elementary resistances: they can be stored in a much more compact way outside of Metadata.
				return;
			}

			var multiplier = FunctionsDatabase.calculate( leveledBonus.levelFunction, tier );
			...
			if ( stat == 'powerMultiplier' ) {
				// Show damage bonus in percents (0.25 => 25%).
				scaledValue *= 100;
			}

			metadata[leveledBonus.stat] = scaledValue;
		} );
		...
		var mechFuel = Query.getMechFuelValue( this.itemCode );
		if ( mechFuel ) {
			metadata.mechFuel = mechFuel;
		}
