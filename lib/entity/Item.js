...
const { LoadedAsset, Query, EntityWithPageName, FunctionsDatabase, MaterialDatabase,
		WeaponAbilityDatabase, AssetDatabase, CargoRow, util } = require( '..' ),
...
class Item {
	...
	constructor( asset, overrideId = '' ) {
		...
		// Obtain human-readable name of shop buildings, staged crafting stations, etc.
		// This name might be from GUI configuration (NOT from "shortdescription" key).
		if ( this.interactData ) {
			...
			if ( layout ) {
				if ( layout.lblTitle ) {
					// New GUI format.
					this.shortdescription = layout.lblTitle.value;
				} else if ( layout.windowtitle ) {
					// Legacy GUI format, might still be used by some stations.
					this.shortdescription = layout.windowtitle.title;
				}
			}
		}
