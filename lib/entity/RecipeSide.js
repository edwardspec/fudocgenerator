...
class RecipeSide {
	...
	*rowIterator() {
	...
	toWikitext( craftingStation ) {
		var { ItemDatabase, MonsterDatabase } = require( '..' );
		var wikitext = '';
		...
		for ( var [ itemCodeWithNamespace, amount ] of this.rowIterator() ) {
			...
			if ( !displayNameWikitext ) {
				...
				var [ , namespace, code ] = itemCodeWithNamespace.match( /^(?:(monster):|)(.*)$/ );
				switch ( namespace ) {
					case 'monster':
						// Name of monster.
						var monster = MonsterDatabase.find( code );
						if ( !monster ) {
							util.log( '[warn] Unknown monster in the recipe: ' + code );
							return '';
						}
					...
					default:
						// Normal item.
						var item = ItemDatabase.find( code );
						if ( !item ) {
							util.warnAboutUnknownItem( code );
							return '';
						}
