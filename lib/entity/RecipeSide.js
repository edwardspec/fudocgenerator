...
class RecipeSide {
	 ...
	 */
	addQuest( questCode ) {
		return this.addItem( 'quest:' + questCode );
	}
	...
	toWikitext( craftingStation ) {
		...
		for ( var [ itemCode, amount ] of this.rowIterator() ) {
			...
			if ( !displayNameWikitext ) {
				...
				} else if ( itemCode.startsWith( 'quest:' ) ) {
					// Quest ID. This is typically a quest that is required to be completed
					// before the recipe becomes available.
					var questCode = itemCode.replace( /^quest:/, '' );
					var quest = QuestDatabase.find( questCode );
					if ( !quest ) {
						util.log( '[warn] Unknown quest in the recipe: ' + questCode );
						return '';
					}

					var questName = quest.title;
					displayNameWikitext = "''(only after completing the quest: '''" + questName + "''')''";
				} else {
					// Normal item.
