...
/**
 * Keys: all supported prefixes of pseudo-items. They are prepended to itemCode, delimited by ":".
 * For example, if itemCode is "monster:poptop", then namespace is "monster".
 * Default namespace (when itemCode doesn't start with any supported prefix) is "item".
 * Values: function that takes itemCode (with prefix already removed) as parameter and returns
 * the wikitext representation of this pseudo-item (normally a wikitext link to article about it).
 */
const itemCodeNamespaceFormatters = {
	item: function ( itemCode ) {
		var { ItemDatabase } = require( '..' );

		// Default namespace: normal items.
		// For example, "bottle" means an item "Empty Bottle" (which can be found by ID "bottle").
	...
	monster: function ( monsterCode ) {
		var { MonsterDatabase } = require( '..' );

		// Monster namespace: pseudo-item that represents a monster.
		// For example, "monster:gleap" means Gleap (which is a monster with ID "gleap").
		var monster = MonsterDatabase.find( monsterCode );
		if ( !monster ) {
...
class RecipeSide {
	 ...
	 */
	getItemCodes() {
		return Object.keys( this.items ).filter( ( itemCode ) => itemCode !== 'PSEUDO_ITEM' );
	}
	...
	*rowIterator() {
	...
	toWikitext( craftingStation ) {
		...
		for ( var [ itemCodeWithNamespace, amount ] of this.rowIterator() ) {
			...
			if ( !displayNameWikitext ) {
				...
				var itemCodeParts = itemCodeWithNamespace.split( ':' ),
					namespace = itemCodeParts.shift(),
					itemCode = itemCodeParts.join( ':' );

				var formatter = itemCodeNamespaceFormatters[namespace];
				if ( !formatter ) {
					// What we assumed to be namespace was a part of item ID, e.g. "prototyper:3".
					itemCode = itemCodeWithNamespace;
					formatter = itemCodeNamespaceFormatters['item'];
				}

				displayNameWikitext = formatter( itemCode );
				if ( !displayNameWikitext ) {
					// Missing item, monster, etc.
					// We return empty string, because this entire RecipeSide object is unusable.
					return '';
				}
			}
