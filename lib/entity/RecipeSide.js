...
class RecipeSide {
	/**
	 * @param {object|RecipeSide} rawList Map of input materials,
	...
	static newFromCraftingInput( nativeCraftingInput ) {
		...
		nativeCraftingInput.forEach( function ( ingredient ) {
			inputs[ingredient.item || ingredient.name] = { count: ingredient.count || 1 };
		} );

		return new RecipeSide( inputs );
	}
	 ...
	 */
	isValid() {
		for ( var [ itemCode, quantityAttributes ] of Object.entries( this ) ) {
