...
class RecipeSide {
	...
	static newFromCraftingInput( nativeCraftingInput ) {
		...
		nativeCraftingInput.forEach( function ( ingredient ) {
			const quantityAttributes = { count: ingredient.count || 1 };
	...
	static newFromExtraction( valuesArray, extractorStage = 0 ) {
		...
		for ( const [ itemCode, counts ] of Object.entries( valuesArray ) ) {
			const count = Number.isInteger( counts ) ? counts : counts[extractorStage];
	...
	/**
	 * Create RecipeSide object from outputs of resource generators like Atmospheric Condenser.
	 * Sample input: [
	 *    { "weight" : "common", "items" : [ "A", "B" ] },
	 *    { "weight" : "uncommon", "items" : [ "C", "D", "E" ] },
	 *    { "weight" : "rare", "items" : [ "F" ] }
	 * ]
	 *
	 * @param {Array} pool
	 * @param {Object} namedWeights Example: { "common" : 79, "uncommon" : 20, "rare" : 1 }
	 * @return {RecipeSide}
	 */
	static newFromResourceGeneration( pool, namedWeights ) {
		const side = new RecipeSide();
		const sumOfWeights = util.sum( Object.values( namedWeights ) );

		for ( const subpool of pool ) {
			let chanceOfOneItem = 100.0 * namedWeights[subpool.weight] / sumOfWeights;
			...
			chanceOfOneItem /= subpool.items.length;

			for ( const itemCode of subpool.items ) {
				side.addItem( itemCode, { chance: chanceOfOneItem } );
			}
		}
	 ...
	 * This can be used with makeItemUnlisted(), etc. to avoid modifying the original RecipeSide object.
	 *
	 * @return {RecipeSide}
	 */
	clone() {
		const cloned = new RecipeSide();
	...
	addComponent( component ) {
		...
		for ( const subcomponent of component.getSubComponents() ) {
	...
	addComponentWithoutSubcomponents( component ) {
		...
		if ( !list ) {
			...
			for ( const existingComponent of list ) {
	 ...
	 * @return {this}
	 */
	addBiome( biomeCode, quantityAttributes = {} ) {
		return this.addComponent( RecipeComponent.newBiome( biomeCode, quantityAttributes ) );
	}
	...
	addSpawnType( spawnTypeName, quantityAttributes = {} ) {
		const spawnType = SpawnTypeDatabase.find( spawnTypeName );
		if ( !spawnType ) {
			util.log( '[error] Unknown spawnType=' + spawnTypeName + ' in the recipe.' );
			return this;
		}

		const daytime = spawnType.spawnParameters.time;
		if ( daytime === 'day' || daytime === 'night' ) {
			quantityAttributes.subtype = daytime + ' only';
		}

		let monsterTypes = spawnType.monsterType,
			singleMonsterType = null;

		if ( !Array.isArray( monsterTypes ) ) {
			singleMonsterType = monsterTypes;
		} else if ( monsterTypes.length === 1 ) {
			singleMonsterType = monsterTypes[0][1];
		}

		if ( singleMonsterType ) {
			...
			return this.addMonster( singleMonsterType, quantityAttributes );
		}

		const components = [];
		const weightedMonsters = util.normalizeWeights( util.flattenWeightedPool( monsterTypes ) );

		for ( const [ chance, monsterCode ] of weightedMonsters ) {
	...
	moveItemToBottom( itemCode ) {
		const components = this.items.get( itemCode );
		if ( components ) {
	...
	addEverythingFrom( anotherRecipeSide ) {
		for ( const [ itemCode, rows ] of anotherRecipeSide.items ) {
	...
	getAllCodes() {
		const codes = [];
		for ( const itemCode of this.items.keys() ) {
	 ...
	 */
	getItemCodes() {
		return this.getAllCodes().filter( ( itemCode ) => !itemCode.match( /^(monster|quest|pool|biome|stem|foliage|tenant):/ ) );
	}
	...
	getPoolNames() {
		const poolCodes = [];
		for ( const itemCode of this.items.keys() ) {
			const match = itemCode.match( /^pool:(.*)$/ );
			if ( match ) {
	...
	toWikitext( renderParameters ) {
		...
		for ( const component of this.getAllComponents() ) {
			if ( component.quantity.hidden ) {
				// This component is not supposed to add any wikitext.
				continue;
			}

			const componentWikitext = component.toWikitext( renderParameters );
