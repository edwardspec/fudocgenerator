...
const { RecipeComponent, SpawnTypeDatabase, LiquidDatabase, util } = require( '..' );
...
class RecipeSide {
	...
	static newFromResourceGeneration( pool, namedWeights ) {
		var side = new RecipeSide();
		var sumOfWeights = util.sum( Object.values( namedWeights ) );

		for ( var subpool of pool ) {
			var chanceOfOneItem = 100.0 * namedWeights[subpool.weight] / sumOfWeights;
	 ...
	 * @param {Object} quantityAttributes
	 * @param {string} overrideItemId Optional string to be used instead of the item ID.
	 * @return {this}
	 */
	addPseudoItem( displayName, quantityAttributes = {}, overrideItemId = null ) {
		return this.addComponent( RecipeComponent.newPseudoItem( displayName, quantityAttributes, overrideItemId ) );
	}
	...
	addSpawnType( spawnTypeName, quantityAttributes = {} ) {
		...
		quantityAttributes.subtype = [];

		var daytime = spawnType.spawnParameters.time;
		if ( daytime === 'day' || daytime === 'night' ) {
			quantityAttributes.subtype.push( daytime + ' only' );
		}

		var params = spawnType.monsterParameters;
		if ( params && params.aggressive === false ) {
			quantityAttributes.subtype.push( 'peaceful' );
		}

		var monsterTypes = spawnType.monsterType,
		...
		var components = [];
		var weightedMonsters = util.normalizeWeights( util.flattenWeightedPool( monsterTypes ) );
	...
	/**
	 * Add 1 liquid to this RecipeSide object.
	 *
	 * @param {int} liquidId
	 * @param {Object} quantityAttributes
	 * @return {this}
	 */
	addLiquid( liquidId, quantityAttributes = {} ) {
		var liquid = LiquidDatabase.find( liquidId );
		if ( !liquid ) {
			util.log( '[error] Unknown liquid=' + liquidId + ' in the recipe.' );
			return;
		}

		return this.addComponent( RecipeComponent.newItem( liquid.itemDrop, quantityAttributes ) );
	}
