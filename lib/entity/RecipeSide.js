...
class RecipeSide {
	...
	/**
	 * Add available amount of goods (for spacestations) to this RecipeSide object.
	 *
	 * @param {number} averageCount
	 * @param {string[]|string|null} stations Where are the goods sold, e.g. "trading" or [ "medical", "military" ].
	 * @return {this}
	 */
	addAvailableAmount( averageCount, stations ) {
		var formattedCount = '~' + Math.round( averageCount );
		if ( !stations ) {
			return this.addComment( 'Normal amount: ' + formattedCount );
		}
		...
		// Exclude scientific stations, they are currently not obtainable in-game.
		stations = stations.filter( ( stationType ) => stationType !== 'scientific' ).join( ', ' );
		if ( stations ) {
			this.addComment( 'Amount (' + stations + '): ' + formattedCount );
		}
