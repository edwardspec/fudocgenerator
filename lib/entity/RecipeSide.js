...
class RecipeSide {
	...
	addSpawnType( spawnTypeName, quantityAttributes = {} ) {
		...
		this.addComment( '<div style="border: 1px solid black; padding: 5px;">One of:' );
		...
		for ( var [ chance, monsterCode ] of weightedMonsters ) {
			this.addMonster( monsterCode, { chance: chance * 100 } );
		}

		var closingComponent = RecipeComponent.newComment( '</div>' );
		for ( var [ attr, value ] of Object.entries( quantityAttributes ) ) {
			closingComponent.setQuantity( attr, value );
		}

		this.addComponent( closingComponent );
		return this;
	}
	 ...
	 */
	getItemCodes() {
		return this.getAllCodes().filter( ( itemCode ) => !itemCode.match( /^(monster|quest|pool|biome):/ ) );
	}
