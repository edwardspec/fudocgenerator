...
class LinearClampFunction {
	...
	constructor( functionName, values ) {
		...
		const levels = values.map( ( pair, index ) => {
			const [ param, value ] = pair,
				derivative = 0;

			const nextPair = values[index + 1];
			if ( nextPair ) {
				const [ nextParam, nextValue ] = nextPair;
				const dx = nextParam - param;
				if ( dx !== 0 ) {
	 ...
	 */
	calculate( param ) {
		const exactMatch = this.values.get( param );
		if ( exactMatch ) {
			return exactMatch.value;
		}

		let maxLowerParam;
		for ( const [ levelParam, levelInfo ] of this.values ) {
			if ( levelParam > param ) {
		...
		const chosenLevelInfo = this.values.get( maxLowerParam );
	...
	toCargoDatabase() {
		const rows = [];
		for ( const [ param, levelInfo ] of this.values ) {
