...
const { CargoRow, PageNameRegistry } = require( '..' );
...
class Monster {
	...
	/**
	 * Get PageName assigned to this Monster by PageNameRegistry, or empty string if none was assigned.
	...
	/**
	 * Make MediaWiki link: either [[NameOfMonster]] or [[NameOfPage|NameOfMonster]] (if they are named differently).
	 *
	 * @return {string}
	 */
	getWikiPageLink() {
	...
	toArticleText( wikiPageName ) {
		return "{{-start-}}\n'''" + wikiPageName + "'''\n" +
			'{{Automatic infobox monster|' + this.type + "}}<!-- Please don't delete this line -->\n" +
