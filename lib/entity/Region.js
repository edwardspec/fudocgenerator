...
class Region {
	...
	constructor( code, rawData ) {
		...
		for ( var minTierBiomes of rawData.biome ) {
			let [ minTier, biomeCodes ] = minTierBiomes;
		...
		for ( var i = 0; i < this.biomeOptions.length - 1; i++ ) {
			let minTier = this.biomeOptions[i].minTier,
	...
	getCodesForTier( minTier, maxTier ) {
		var seenBiomes = new Set(); // { biomeCode1, biomeCode2, ... }
	...
	getDayLightColors() {
		var possibleColors = [];
		for ( var biomeOption of this.biomeOptions ) {
			var biome = Query.findBiome( biomeOption.biomeCode );
			possibleColors.push( biome.getDayLightColors() );
		}
