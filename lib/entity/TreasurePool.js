...
class TreasurePool {
	...
	constructor( poolName, poolData ) {
		...
		let poolRounds = poolData.poolRounds;
		if ( !poolRounds ) {
			poolRounds = [ [ 1, 1 ] ];
		...
		poolRounds.forEach( ( poolRoundsOption ) => {
			...
			const [ chance, numberOfRounds ] = poolRoundsOption;
			averageNumberOfRounds += chance * numberOfRounds;
		} );

		let poolElements = poolData.pool || [],
			sumOfWeights = 0;
		...
		const weightMultiplier = averageNumberOfRounds / ( sumOfWeights || 1 );
		poolElements.forEach( ( poolElement ) => {
