...
class Recipe {
	...
	constructor( station, inputs, outputs, context = {} ) {
		...
		if ( !this.type ) {
			// Type is not strictly necessary, but knowing it improves output of DiffableResult,
			// so we require it anyway.
	...
	/**
	 * Get type of this Recipe as string (for logging purposes).
	 *
	 * @return {string}
	 */
	getTypeAsString() {
	...
	getSomewhatUniqueId() {
		...
		switch ( this.type ) {
			...
			case Recipe.Type.Shiplocker:
				// Contents of species-specific starting shiplocker are identified by the ID of the species.
				return this.inputs.getAllComponents()[0].quantity.subtype;
			...
			case Recipe.Type.BiomeContents:
				...
				component = this.inputs.getAllComponents()[0];
				return this.station + '-' + component.id + '-' + component.quantity.subtype;
		...
		throw new Error( 'getSomewhatUniqueId(): not yet implemented for recipe type: ' + this.type.toString() );
	}
