...
class Biome {
	...
	constructor( rawData ) {
		...
		this.id = this.name;
		this.displayName = Query.getBiomeName( this.id ) || this.friendlyName || this.id;
	 ...
	 */
	toArticleText() {
		return '{{Automatic infobox biome|' + this.type + "}}<!-- Please don't delete this line -->\n" +
			'<!-- You can write the text below. -->\n\n\n' +
			'{{All recipes for biome|' + this.type + '}}' +
