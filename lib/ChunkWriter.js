...
class ChunkWriter {
	...
	finalize( filename, headerText, footerText, averageChunkSizeBytes ) {
		...
		// Load the cache with the results of partitionSelector.getNode().
		// This is a very time-intensive operation (multiple checksum calculations for every entry),
		// and it returns the same value for the same partitionKey (if the number of chunks hasn't changed),
		// so the cache can help a lot.
		// Format: { partitionKey: partitionName, ... }.
		var cache = {},
			cacheFilename = os.tmpdir() + '/fudocgenerator/chunkNameToPartition.cache',
			cacheOutdated = true;
		...
		// Statistics: partitionSelector.getNode() is relatively slow, so measure how much time it took.
		var secondsSpent,
			startTime = Date.now();
		...
		this.entries.forEach( ( entry, idx ) => {
			...
			var chunkName = cache[partitionKey];
			if ( !chunkName ) {
				chunkName = partitionSelector.getNode( partitionKey );
				cache[partitionKey] = chunkName;
