...
class ChunkWriter {
	/**
	 * @param {Object} chunkGroupsConfig
	 * Required configuration keys:
	 * {string} filename Name of output file. This file will be overwritten.
	 * {string} headerText Can contain "$1" (will be replaced with the number of chunk).
	 * {string} footerText
	 * {int} maxChunkSizeBytes Hard limit on the size of chunk (chunk will be split if this is exceeded).
	 * ... and where "groups" has format { "groupName": { ... group configuration ...} },
	 * ... where group configuration is Object with the following keys:
	 ...
	 */
	constructor( chunkGroupsConfig ) {
		// Copy fields like "filename", "header", "groups", etc. from chunkGroupsConfig.
		Object.assign( this, chunkGroupsConfig );

		// Every call of write() results in 1 additional element in "entries" array.
		// Format: [ { partitionKey: "partitionKey1", text: "text1" }, ... ]
		Object.values( this.groups ).forEach( ( group ) => group.entries = [] );
	}
	...
	finalize() {
		...
		Object.keys( this.groups ).forEach( ( groupName ) => this.finalizeGroup( groupName, fd ) );
		fs.closeSync( fd );
	}
	...
	finalizeGroup( groupName, fd ) {
		...
		var totalSize = 0;
		group.entries.forEach( ( entry ) => { totalSize += entry.text.length; } );
		...
		var writeHeader = ( suffix ) => {
			...
			var chunkHeaderText = this.headerText.replace( /\$1/g,
				group.idxPattern.replace( /\$1/g, currentChunkName ) );
			fs.writeSync( fd, chunkHeaderText );

			// Each call to writeHeader() begins the new chunk.
			bytesWritten[currentChunkName] = chunkLength = 0;
		};
		var writeFooter = () => {
			fs.writeSync( fd, this.footerText );
		...
		chunkPartitions.forEach( ( entryIds, chunkIndex ) => {
			...
			entryIds.map( ( id ) => group.entries[id].text ).forEach( ( text ) => {
				if ( chunkLength + text.length > this.maxChunkSizeBytes ) {
