...
class ChunkWriter {
	...
	finalize( filename, headerText, footerText, averageChunkSizeBytes ) {
		...
		for ( var [ chunkName, entryIds ] of Object.entries( chunkPartitions ) ) {
			...
			kbytesWritten[chunkName] += Math.round( chunkLength / 1024 );
		}

		fs.closeSync( fd );

		util.log( '[debug] ChunkWriter: sizes of chunks (KB): ' +
			Object.values( kbytesWritten ).sort( ( a, b ) => a - b ).join( ', ' ) );
	}
}
