...
class ChunkWriter {
	 ...
	 * @param {Function} groupsConfig.outputCallback Function that is called when each chunk is ready.
	 * It receives the following parameters: 1) chunk name (string), 2) contents of chunk (string).
	 */
	constructor( groupsConfig ) {
		// Copy fields like "outputCallback", "groups", etc. from groupsConfig.
	 ...
	 */
	finalize() {
		Object.keys( this.groups ).forEach( ( groupName ) => this.finalizeGroup( groupName ) );
	}
	...
	finalizeGroup( groupName ) {
		...
		var writeFooter = () => {
			this.outputCallback( currentChunkName, currentChunkContents );
