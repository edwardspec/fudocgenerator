 ...
 */
const filenameGlob = '**/{*.{config,item,liqitem,matitem,consumable,currency,object,thrownitem,activeitem,augment,chest,' +
	'head,legs,back,*tool,flashlight,beamaxe,recipe,treasurepools,monstertype,liquid,material,biome}' +
	',crewmembergeologist.npctype}';
...
class AssetDatabase {
	...
	discoverFilesInDirectory( directory ) {
		...
		glob.sync( globPattern ).forEach( ( filename ) => {
			...
			if ( this.knownAssets.has( relativeFilename ) && isVanilla && !this.knownAssets.get( relativeFilename ).vanilla ) {
			...
			this.knownAssets.set( relativeFilename, {
	...
	async parseDiscoveredFiles() {
		...
		for ( var [ filename, asset ] of this.knownAssets ) {
			...
			var promise = new Promise( async function ( resolve, reject ) {
				var loadedData = null;
				try {
					loadedData = await util.loadModFile( this.asset.absolutePath );
				} catch ( error ) {
					// Ignore incorrect files.
					util.log( "[warning] Failed to load JS file: " + this.asset.absolutePath + ": " +
						error.message + "\n" +  error.stack.split( "\n" ).slice( 1, 3 ).join( "\n" ) );
				}

				resolve( [ this.filename, loadedData ] );
			}.bind( { filename: filename, asset: asset } ) );

			allPromises.push( promise );
		}

		for ( var [ filename, loadedData ] of await Promise.all( allPromises ) ) {
			if ( loadedData ) {
				this.knownAssets.get( filename ).data = loadedData;
			} else {
	...
	/**
	 * Get the parsed data by path. Throws an exception if the asset doesn't exist.
	 * @param string path Relative path to asset (e.g. "/interface/windowconfig/craftingmech.config").
	 * @return Object|undefined
	 */
	getData( path ) {
		...
		if ( !asset ) {
			throw new Error( 'AssetDatabase.getData(): no such asset: ' + path );
		}
