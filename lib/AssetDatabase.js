		...
		for ( var filename of modifiedFiles ) {
			...
			if ( !fs.existsSync( config.pathToMod + '/' + filename ) ) {
				...
				if ( this.knownAssets.has( filename ) ) {
					...
					// New asset was added.
					// TODO: performance: this doesn't apply the "ignore" patterns from discoverFilesInDirectory(),
					// so we may unnecessarily parse an asset like [slimepersonnamegen.config] (which we don't use).
					...
					if ( lazyAsset ) {
						...
						// This file was added to Git, but it not a JSON asset (e.g. a "png" image).
						changedAssetsCount --;
					}
				}
			}
