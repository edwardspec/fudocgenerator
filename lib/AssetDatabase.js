 ...
 */
const filenameGlob = '**/*.{config,item,liqitem,matitem,consumable,currency,object,thrownitem,activeitem,augment,chest,head,legs,back,*tool,flashlight,beamaxe,recipe,treasurepools,monstertype,liquid,material,biome,species}';
...
class AssetDatabase {
	...
	discoverFilesInDirectory( directory ) {
		...
		var globOptions = {
			cwd: directory,
			ignore: [
				// We don't currently use projectile files, and they are quite large, so let's skip them.
				'projectiles/**/*.config',
				'species/*namegen.config'
			]
		};
	...
	parseDiscoveredFiles() {
		...
		for ( var [ filename, asset ] of this.knownAssets ) {
			...
			var loadedData = util.loadModFile( asset.absolutePath );
			if ( !loadedData ) {
				// Failed to load.
