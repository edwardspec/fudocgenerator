 ...
 */
const filenameGlob = '**/*.{config,item,liqitem,matitem,consumable,currency,object,thrownitem,activeitem,augment,chest,head,legs,back,*tool,flashlight,beamaxe,recipe,treasurepools,monstertype,liquid,material,biome}';
...
class AssetDatabase {
	...
	parseDiscoveredFiles() {
		...
		for ( var [ filename, asset ] of this.knownAssets ) {
			...
			var asset = this.knownAssets.get( filename );
			asset.data = loadedData;
			this.knownAssets.set( filename, asset );
		}
	}
	...
	applyPatches() {
		for ( var [ filename, asset ] of this.knownAssets ) {
			...
			var asset = this.knownAssets.get( targetFilename );
			asset.patched = true;
			this.knownAssets.set( targetFilename, asset );
		}
	}
