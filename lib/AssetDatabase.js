...
class AssetDatabase {
		...
		if ( !this.matcher ) {
			...
			var opts = {
				ignore: [
					...
					'a_modders/**',
					// Wild seeds (skipped by ItemDatabase anyway).
					'**/wild*seed.object{,.patch}',
					// Directories with non-assets
					'tests/**'
				]
			};
	...
	discoverFilesInDirectory( directory ) {
		...
		for ( var entry of fsWalkSync( directory, opts ) ) {
			if ( !entry.dirent.isDirectory() ) {
				var filename = entry.path.slice( directory.length + 1 );
