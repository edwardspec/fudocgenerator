...
class AssetDatabase {
	constructor() {
		...
		// Assets groupped by asset.type.
		// Format: { "item": [ { filename1, LoadedAsset1 }, ... ], "monster": [ ... ], ... }
		this.knownAssetsByType = { unknown: [] };
	}
	...
	load() {
		...
		this.populatePerTypeAssetLists();
	...
	/**
	 * Sort the assets to make forEach(type) faster.
	 */
	populatePerTypeAssetLists() {
		for ( var type of Object.values( config.extensionToAssetType ) ) {
	...
	forEach( assetType, callback ) {
		...
		for ( var entry of ( this.knownAssetsByType[assetType] || [] ) ) {
			callback.apply( null, entry );
		}
	}
