...
class AssetDatabase {
	...
	/**
	 * Enable "load only vanilla assets" mode.
	 * Can be used by tools like [export_database.js] to export lists of vanilla liquids/materials (for unit tests).
	 */
	enableVanillaOnlyMode() {
		if ( this.loaded ) {
			throw new Error( "AssetDatabase.enableVanillaOnlyMode() can't be called after load()." );
		}
	 ...
	 */
	load() {
		if ( this.vanillaOnly || !this.loadFromCache() ) {
	...
	loadWithoutCache() {
		...
		this.mustUpdateCache = !this.vanillaOnly;
