...
class AssetDatabase {
	 ...
	 */
	load( options = {} ) {
		const canUseCache = !options.vanillaOnly;
		...
		for ( const type of Object.values( config.extensionToAssetType ).concat( [ 'unknown' ] ) ) {
			this.knownAssetsByType[type] = [];
		}

		for ( const [ filename, asset ] of this.knownAssets ) {
	...
	loadFromCache() {
		...
		const prevCommit = this.getCachedCommit();
		if ( !prevCommit ) {
			// Cache doesn't exist.
			return false;
		}

		const modifiedFiles = this.getFilesModifiedInGit( prevCommit );
		...
		for ( const assetInfo of cachedAssets ) {
		...
		let changedAssetsCount = modifiedFiles.length;
		for ( const filename of modifiedFiles ) {
			const targetOfPatch = filename.replace( /\.patch$/, '' );
			const isPatch = ( filename !== targetOfPatch );
			...
			const patchFilename = isPatch ? filename : ( filename + '.patch' );
			...
			if ( isPatch ) {
				...
				const targetAsset = this.knownAssets.get( targetOfPatch );
				if ( targetAsset ) {
			...
			if ( !fs.existsSync( config.pathToMod + '/' + filename ) ) {
				...
				if ( this.knownAssets.has( filename ) ) {
					...
					// New asset was added.
					const lazyAsset = this.addAssetByFilename( filename, config.pathToMod, false );
					if ( lazyAsset ) {
						lazyAsset.loadNow();
	...
	addAssetByFilename( filename, directory, isVanilla ) {
		...
		const lazyAsset = new LoadedAsset( this, assetInfo );
		if ( type === 'patch' ) {
	...
	discoverFilesInDirectory( directory ) {
		const isVanilla = ( directory === config.pathToVanilla );
		...
		let files = this.listAllFilesInGit( directory );
		if ( !files ) {
	...
	applyPatchInstructions( patch, data ) {
		...
		patch.forEach( ( instruction ) => {
			...
			const objectPath = instruction.path.slice( 1 ).split( '/' );
			if ( op === 'add' ) {
				...
				const possiblyIndex = objectPath.pop();
				...
				if ( possiblyIndex !== '-' && Number.isNaN( Number( possiblyIndex ) ) ) {
					...
					const currentArray = ( lodash.get( data, objectPath ) || [] );
					...
					if ( possiblyIndex === '-' ) {
						...
						const before = currentArray.slice( 0, possiblyIndex ),
			...
			if ( op === 'replace' || op === 'add' ) {
				...
				const index = objectPath.pop();
				const list = lodash.get( data, objectPath );
				...
				const fieldExists = lodash.has( data, objectPath );
	...
	loadExtra( path ) {
		...
		let isVanilla, directory, absolutePath;
		let found = false;
