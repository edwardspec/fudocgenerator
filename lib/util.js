...
util.addContextToJsonError = function ( exception, sourceCode ) {
	...
	const match = errorMessage.match( 'at position ([0-9]+)' );
	if ( match ) {
...
util.loadModFile = function ( filename ) {
	...
	const sanitizedJson = util.sanitizeRelaxedJson( fs.readFileSync( filename ).toString() );
	let result;
...
util.removeIrelevantAssetKeys = function ( data ) {
	for ( const keyToDelete of config.ignoredAssetKeys ) {
		delete data[keyToDelete];

		if ( data.upgradeStages ) {
			for ( const stage of data.upgradeStages ) {
...
util.findInModOrVanilla = function ( relativePath ) {
	...
	for ( const possiblePath of pathCandidates ) {
...
util.trimFloatNumber = function ( number, digitsAfterComma ) {
	const divisor = 10 ** digitsAfterComma;
...
util.describeDistribution = function ( arbitraryValues, optionalSuffixAfterValue = '' ) {
	const useCount = {}; // { value: numberOfOccurences1, ... }
	...
	return Object.entries( useCount )
		.sort( ( a, b ) => b[1] - a[1] )
		.map( ( [ value, uses ] ) => {
			const percent = Math.ceil( 100 * uses / arbitraryValues.length );
...
util.flattenWeightedPool = function ( weightedOptions ) {
	...
	for ( const [ weight, value ] of weightedOptions ) {
		if ( Array.isArray( value ) ) {
			// This subpool can have other subpools inside,
			// so let's unwrap it recursively.
			const subpool = util.flattenWeightedPool( value );
			for ( const [ subweight, stringValue ] of subpool ) {
	...
	const values = {}; // { value1: weight1, ... }
	for ( const [ weight, value ] of unwrappedOptions ) {
		values[value] = ( values[value] || 0 ) + weight;
	}

	const unwrappedOptionsUnique = [];
...
const trivialAbilities = new Set( config.trivialWeaponAbilities );
...
util.getAttackMetadata = function ( attack, prefix, damageMultiplier ) {
	...
	if ( dps ) {
		let fireTime = attack.fireTime || 1;
