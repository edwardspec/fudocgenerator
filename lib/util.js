...
util.loadModFile = function ( filename ) {
	...
	if ( !cacheUpdatedTouchFile ) {
		cacheUpdatedTouchFile = util.tmpdir + '/VmScriptCache/.updated';
	...
	var cachedFilename = util.tmpdir + '/VmScriptCache' + filename + '.cache';
...
util.getAttackMetadata = function ( attack, prefix, damageMultiplier ) {
	...
	if ( attack.baseDps ) {
		...
		metadata.damagePerHit = util.trimFloatNumber( damageMultiplier * attack.baseDps * fireTime, 1 );
	}
