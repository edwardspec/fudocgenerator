...
module.exports.researchNodeToCargoDatabase = function ( node ) {
	...
	var childrenWikitext = node.children.map( ( childNodeId ) => {
		var childNode = ResearchTreeDatabase.find( node.tree, childNodeId );
		if ( !childNode ) {
			util.log( '[error] Unknown child node: ' + childNodeId + ' (' + node.tree + ' tree)' );
			return false;
		}

		return childNode.wikiPageLink;
	} ).filter( ( x ) => x !== false ).join( ', ' );
