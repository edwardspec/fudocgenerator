...
var config = require( '../config.json' ),
	...
	ResearchTreeDatabase = require( './ResearchTreeDatabase' );
...
module.exports.loadModFile = function ( filename ) {
	...
	if ( fs.existsSync( cachedFilename ) &&
		fs.statSync( cachedFilename ).mtimeMs > fs.statSync( filename ).mtimeMs
	) {
		// Found serialized result in cache.
		// This is much faster than executing vm.runInThisContext().
		return jsonpack.unpack( fs.readFileSync( cachedFilename ).toString() );
	}
	...
	var result = vm.runInThisContext( '_ThisVariableIsNeverUsed = ' + sanitizedJsCode );

	// Add to cache.
	fs.mkdirSync( path.dirname( cachedFilename ), { recursive: true } );
	fs.writeFileSync( cachedFilename, jsonpack.pack( result ) );
 ...
 */
module.exports.trimFloatNumber = function ( number, digitsAfterComma ) {
 ...
 * e.g. [ "psionics2", "cosmic2" ].
 * @param {string} treeName Human-readable tree name, e.g. "Geology".
 * @param {string[]} craftingStation
 * @return {string}
 */
module.exports.researchNodeListToWikitext = function ( treeName, nodeIdsArray ) {
	return nodeIdsArray.map( ( nodeId ) => {
		var node = ResearchTreeDatabase.find( treeName, nodeId );
		if ( !node ) {
...
module.exports.researchNodeToCargoDatabase = function ( node ) {
	...
	var unlocksWikitext = node.unlocks.map( ( itemCode ) => {
	...
	var childrenWikitext = util.researchNodeListToWikitext( node.tree, node.children ),
		parentsWikitext = util.researchNodeListToWikitext( node.tree, node.parents );
