...
module.exports.ingredientsListToWikitext = function ( ingredientsMap ) {
	...
	for ( var [ itemName, amount ] of Object.entries( ingredientsMap ) ) {
		...
		if ( amount.rarity ) {
			// TODO: calculate and show individual chances for each centrifuge.
			// For now we can show rarity and chanceDivisor (greater number means "more rare"),
			// since it's helpful enough for reader to assess "how rare is this output".
			var [ rarity, chanceDivisor ] = amount.rarity;
			wikitext += " ''(" + rarity;
			// wikitext += '/' + chanceDivisor; // Not shown for now
			wikitext += ")''";
		}
