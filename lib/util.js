 ...
 * @return {string}
 */
module.exports.sanitizeRelaxedJson = function ( relaxedJson ) {
...
module.exports.itemToCargoDatabase = function ( item ) {
	...
	wikitext += '|wikiPage=' + item.wikiPageName + '\n';
	...
	if ( item.itemTags ) {
		wikitext += '|tags=' + item.itemTags.join( ',' ) + '\n';
	}

	if ( item.colonyTags ) {
		wikitext += '|colonyTags=' + item.colonyTags.join( ',' ) + '\n';
	}
