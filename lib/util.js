...
const { config, CacheSerialization } = require( '.' ),
	fs = require( 'fs' ),
	os = require( 'os' ),
	path = require( 'path' ),
...
util.loadModFile = function ( filename ) {
	...
	var dataType = filename.split( '.' ).pop(); // For optimized CacheSerialization
	...
	if ( fs.existsSync( cachedFilename ) && cacheUpdatedMtime > fs.statSync( filename ).mtimeMs ) {
		...
		return CacheSerialization.deserialize( fs.readFileSync( cachedFilename ).toString(), dataType );
	}
	...
	try {
		...
		var errorMessage = '[warning] Failed to load JS file: ' + filename + ': ' + error.message;

		// Make the message more helpful by quoting the part of JSON before/after the incorrect symbol.
		var match = error.message.match( 'at position ([0-9]+)' );
		if ( match ) {
			var position = parseInt( match[1] ),
				symbolsToQuote = 100,
				quoteBefore = sanitizedJson.substring( 0, position ).replace( /\s+/g, ' ' ),
				quoteAfter = sanitizedJson.substring( position ).replace( /\s+/g, ' ' );
	...
	fs.writeFileSync( cachedFilename, CacheSerialization.serialize( result, dataType ) );
