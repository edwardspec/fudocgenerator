...
var cacheUpdatedTouchFile = os.tmpdir() + '/fudocgenerator/VmScriptCache/.updated';

/**
 * Modification time of cacheUpdatedTouchFile (see above).
 * Only checked once (at the first loadModFile).
 */
var cacheUpdatedMtime = fs.existsSync( cacheUpdatedTouchFile ) ? fs.statSync( cacheUpdatedTouchFile ).mtimeMs : 0;

/**
 * Load the contents of one *.config/*.object file from the Starbound mod.
 * Returns the parsed structure.
 */
util.loadModFile = async function ( filename ) { return new Promise( ( resolve, reject ) => {
	if ( filename[0] !== '/' ) {
	...
	var loadFromCachePromise = new Promise( ( resolve2, reject2 ) => {
		fsPromises.stat( cachedFilename ).catch( () => false ).then( ( stats ) => {
			if ( stats.mtimeMs > cacheUpdatedMtime ) {
				resolve2( false );
				return;
			}

			fsPromises.readFile( cachedFilename )
				.then( ( jsonBuffer ) => {
					resolve2( JSON.parse( jsonBuffer.toString() ) );
				} )
				.catch( reject2 );
		} );
	} );

	loadFromCachePromise.then( ( result ) => {
		// Found in cache.
		resolve( result );
	} ).catch( () => {
