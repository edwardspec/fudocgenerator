...
module.exports.ingredientsListToWikitext = function ( ingredientsMap, craftingStation ) {
	...
	for ( var [ itemName, amount ] of Object.entries( ingredientsMap ) ) {
		...
		var displayNameWikitext;

		if ( itemName === 'PSEUDO_ITEM' ) {
			// This is not an item, but an entry like "Air (on Desert planets)".
			// They are intended to be used for Atmospheric Condenser, etc.
			// Such items must provide their own wikitext.
			displayNameWikitext = amount.wikitext;
			if ( !displayNameWikitext ) {
				// Sanity check to catch typos in our code.
				// This can't be caused by incorrect input files, so we are not silently ignoring this.
				throw new Error( 'Found a pseudo-item without amount.wikitext.' );
			}
		} else {
			// Normal item.
