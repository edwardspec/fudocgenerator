 ...
 * @return {string}
 */
util.describeDistribution = function ( arbitraryValues ) {
	var useCount = {}; // { value: numberOfOccurences1, ... }
	for ( var value of arbitraryValues ) {
...
util.flattenWeightedPool = function ( weightedOptions ) {
	...
	for ( var [ weight, value ] of weightedOptions ) {
		if ( Array.isArray( value ) ) {
			// This subpool can have other subpools inside,
			// so let's unwrap it recursively.
			var subpool = util.flattenWeightedPool( value );
			for ( var [ subweight, stringValue ] of subpool ) {
	...
	for ( [ weight, value ] of unwrappedOptions ) {
		values[value] = ( values[value] || 0 ) + weight;
	}

	var unwrappedOptionsUnique = [];
	for ( [ value, weight ] of Object.entries( values ) ) {
