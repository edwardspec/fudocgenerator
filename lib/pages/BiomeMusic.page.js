...
class BiomeMusicPage {
	...
	getText() {
		let wikitext = '<noinclude><span style="font-size: 150%; font-weight: bold;">' +
		...
		BiomeDatabase.forEach( ( biome ) => {
			...
			for ( const [ daytime, musicConf ] of Object.entries( biome.musicTrack || {} ) ) {
				const outputs = new RecipeSide();
				for ( const track of musicConf.tracks.sort() ) {
