...
class MonsterDatabase {
	constructor() {
		...
		// Array of monsters by display name,
		// e.g. { "Gas Bag": [ Monster1, Monster2, ... ], "Poptop": [ ... ], ... }
		this.knownMonstersByName = new Map();
	}
	...
	load() {
		...
		for ( var [ monsterCode, monster ] of this.knownMonsters ) {
			...
			if ( displayName ) {
				displayName = util.cleanDescription( displayName );
				monster.displayName = displayName;

				var monstersWithThisName = this.knownMonstersByName.get( displayName );
				if ( !monstersWithThisName ) {
					monstersWithThisName = [];
					this.knownMonstersByName.set( displayName, monstersWithThisName );
				}
				monstersWithThisName.push( monster );
			} else {
		...
		AssetDatabase.forEach( 'monsterpart', ( filename, asset ) => {
			...
			var monster = this.knownMonsters.get( data.category );
			if ( !monster ) {
