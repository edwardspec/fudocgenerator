	...
	load() {
		...
		AssetDatabase.forEach( 'item', ( filename, asset ) => {
			var loadedData = asset.data,
				itemCode = loadedData.itemName || loadedData.objectName;
			if ( !itemCode ) {
			...
			if ( loadedData.hasObjectItem === false || itemCode.match( /^wild.*seed$/ ) ) {
			...
			// TODO: move most of this logic into the constructor of Item class.
			...
			var icon = loadedData.inventoryIcon;
			if ( icon && typeof( icon ) === 'string' && !icon.match( ':' ) ) {
				...
				if ( fs.existsSync( iconPath ) ) {
					loadedData.inventoryIconPath = iconPath;
				} else {
					util.log( '[warning] Item ' + itemCode + ' refers to nonexistent inventoryIcon: ' + iconPath );
				}
			}
			...
			if ( loadedData.builder && loadedData.level !== undefined &&
				loadedData.builder.match( /\/(fubuildarmor|buildunrandweapon)\.lua$/ )
			) {
				loadedData.price *= ( 0.5 + loadedData.level / 2 );
			}

			this.knownItems[itemCode] = new Item( loadedData );
		} );
