...
class ItemDatabase {
	constructor() {
		...
		// Map of item names: { wikiPageName: [ itemCode1, itemCode2, ... ], ... }.
		// This is used in findCodeByPageName(),
		// and also in situations when multiple items compete for the same title (e.g. "Ancient Artifact")
		// and should be automatically renamed (if possible).
		this.pageNameToItemCodes = new Map();
	}
	...
	addToKnown( item ) {
		...
		var mapElem = this.pageNameToItemCodes.get( item.wikiPageName );
		if ( !mapElem ) {
			mapElem = [];
			this.pageNameToItemCodes.set( item.wikiPageName, mapElem );
		}
		mapElem.push( item.itemCode );
	}
	...
	load() {
		...
		config.ignoredItems.forEach( ( itemCode ) => {
			shouldIgnoreItem[itemCode] = true;
		} );
		...
		AssetDatabase.forEach( 'item', ( filename, asset ) => {
			...
			if ( item.hasObjectItem === false ) {
				// These are objects like Precursor obelisks or wild seeds.
		...
		for ( var [ wikiPageName, itemCodes ] of this.pageNameToItemCodes ) {
			...
			itemCodes.forEach( ( itemCode ) => {
				...
				} else if ( item.wikiPageName !== oldPageName ) {
			...
			if ( itemCodes.length - solvedDuplicatesCount > 1 ) {
				util.log( '[debug] Item with duplicate name: ' + wikiPageName + ': ' + itemCodes.join( ', ' ) );
			}
		}
	...
	findCodeByPageName( wikiPageName ) {
		var itemCodes = this.pageNameToItemCodes.get( wikiPageName ) || [];
		return itemCodes ? itemCodes.pop() : false;
	}
}
