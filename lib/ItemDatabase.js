...
class ItemDatabase {
	 ...
	 */
	addToKnown( item ) {
		if ( !this.pageNameToItemCodes[item.wikiPageName] ) {
			this.pageNameToItemCodes[item.wikiPageName] = [];
		}

		this.pageNameToItemCodes[item.wikiPageName].push( item.itemCode );
	...
	load() {
		...
		Object.keys( this.pageNameToItemCodes ).forEach( ( wikiPageName ) => {
			var itemCodes = this.pageNameToItemCodes[wikiPageName];
