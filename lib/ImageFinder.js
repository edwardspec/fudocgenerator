...
class ImageFinder {
	...
	unpackSprite( absolutePathToSprite ) {
		let result = this.unpackedSprites.get( absolutePathToSprite );
		if ( !result ) {
	...
	unpackSpriteUncached( absolutePathToSprite ) {
		...
		const framesConf = this.loadFramesConf( absolutePathToSprite );
		if ( !framesConf ) {
		...
		let cachePath = util.tmpdir + '/sprite' + absolutePathToSprite,
			canUseCache = false;
		...
		if ( grid ) {
			...
			for ( const [ index, frameName ] of Object.entries( ( grid.names || [] ).flat() ) ) {
				const path = cachePath + '/icon' + index + '.png';
			...
			for ( let index = 1; index <= entries.length; index++ ) {
				const [ frameName, geometry ] = entries[index - 1];
				const [ x1, y1, x2, y2 ] = geometry.map( ( val ) => parseInt( val ) ),
					width = x2 - x1,
					height = y2 - y1,
		...
		for ( const [ alias, origFrameName ] of Object.entries( framesConf.aliases || {} ) ) {
	...
	loadFramesConf( absolutePathToSprite ) {
		...
		const relativePath = absolutePathToSprite
			.replace( config.pathToVanilla, '' )
			.replace( config.pathToMod, '' )
			.replace( /^\//, '' );

		const relativeDir = nodePath.dirname( relativePath ),
		...
		let stepsUp = relativeDir.split( '/' ).length,
			parentDir = '';
		...
		for ( const possibleFilename of filenameCandidates ) {
			const asset = AssetDatabase.loadExtra( nodePath.join( relativePath, possibleFilename ) );
			if ( asset ) {
	...
	locateImage( relativePath, relativeToAsset ) {
		...
		const [ relativeFilename, ...transformations ] = relativePath.split( '?' );
		let [ path, frame ] = relativeFilename.split( ':' );
		if ( path[0] !== '/' ) {
		...
		let possiblePath = util.findInModOrVanilla( path );
		if ( !possiblePath ) {
		...
		// Found the image!
		if ( frame ) {
			// Replace variables like "<color>" with "default" (we don't know which colors are available,
			// but most .frames files will have "default" as a valid value for every variable that they use).
			frame = frame.replace( /<[^<>]+>/g, 'default' );
			possiblePath = this.unpackSprite( possiblePath )[frame] || false;
		}

		if ( transformations.length ) {
			util.log( '[notice] Image ' + path + ' is skipped: it needs transformations (' +
				transformations.join( '?' ) + '), but we don\'t support them yet.' );
			return false;
		}
